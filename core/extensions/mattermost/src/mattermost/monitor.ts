...
export async function monitorMattermostProvider(opts: MonitorMattermostOpts = {}): Promise<void> {
  ...
  if (slashEnabled) {
    try {
      ...
      // Use the *runtime* listener port when available (e.g. `ultraclaw gateway run --port <port>`).
      // The gateway sets ULTRACLAW_GATEWAY_PORT when it boots, but the config file may still contain
      // a different port.
      const envPortRaw = process.env.ULTRACLAW_GATEWAY_PORT?.trim();
      const envPort = parseStrictPositiveInteger(envPortRaw);
      const slashGatewayPort = envPort ?? cfg.gateway?.port ?? 18790;
