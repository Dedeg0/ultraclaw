...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__ultraclaw_channel_id";
...
type InteractionCallbackConfig = Pick<UltraClawConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "ultraclaw-mattermost-interactions").update(botToken).digest("hex");
}
