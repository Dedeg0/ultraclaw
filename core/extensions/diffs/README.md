...
Read-only diff viewer plugin for **UltraClaw** agents.
...
Set plugin-wide defaults in `~/.ultraclaw/ultraclaw.json`:
...
Before:
UltraClaw supports plugins.

After:
UltraClaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/ultraclaw-diffs`).
