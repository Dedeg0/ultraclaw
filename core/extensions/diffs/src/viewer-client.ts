...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-ultraclaw-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-ultraclaw-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.ultraclawDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.ultraclawDiffsError = "true";
