...
import type { UltraClawPluginApi, UltraClawPluginService } from "ultraclaw/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: UltraClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: UltraClawPluginApi["config"]): string[] {
...
export default function register(api: UltraClawPluginApi) {
  ...
  const timerService: UltraClawPluginService = {
