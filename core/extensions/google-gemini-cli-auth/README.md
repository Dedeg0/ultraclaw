# Google Gemini CLI Auth (UltraClaw plugin)
...
```bash
ultraclaw plugins enable google-gemini-cli-auth
```
...
```bash
ultraclaw models auth login --provider google-gemini-cli --set-default
```
...
- `ULTRACLAW_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `ULTRACLAW_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
