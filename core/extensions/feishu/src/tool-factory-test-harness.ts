import type { AnyAgentTool, UltraClawPluginApi } from "ultraclaw/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: UltraClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<UltraClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
