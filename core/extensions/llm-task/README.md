...
`ultraclaw.invoke --each`) without adding new UltraClaw code per workflow.
...
This extension depends on UltraClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** UltraClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.ultraclaw/extensions` as a standalone plugin directory.
