...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["ultraclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["ultraclaw", "pollId"]) ??
    readNestedString(value, ["ultraclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "ultraclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "ultraclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          ultraclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "ultraclaw poll vote",
          displayText: "Vote recorded",
          value: { ultraclawPollId: pollId, pollId },
        },
      },
    ],
  };
