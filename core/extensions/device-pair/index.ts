...
import type { UltraClawPluginApi } from "ultraclaw/plugin-sdk/device-pair";
import {
...
} from "ultraclaw/plugin-sdk/device-pair";
...
function resolveGatewayPort(cfg: UltraClawPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.ULTRACLAW_GATEWAY_PORT?.trim()) ??
...
function resolveAuthLabel(cfg: UltraClawPluginApi["config"]): ResolveAuthLabelResult {
...
async function resolveGatewayUrl(api: UltraClawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the UltraClaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the UltraClaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",
