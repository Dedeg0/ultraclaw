import {
  ...
  type RuntimeEnv,
} from "ultraclaw/plugin-sdk/nextcloud-talk";
...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as UltraClawConfig);
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as UltraClawConfig,
    surface: CHANNEL_ID,
  });
  const useAccessGroups =
    (config.commands as Record<string, unknown> | undefined)?.useAccessGroups !== false;
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as UltraClawConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as UltraClawConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as UltraClawConfig);
