...
import {
  ...
  type WizardPrompter,
} from "ultraclaw/plugin-sdk/twitch";
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: ULTRACLAW_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: UltraClawConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var ULTRACLAW_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.ULTRACLAW_TWITCH_ACCESS_TOKEN?.trim();
