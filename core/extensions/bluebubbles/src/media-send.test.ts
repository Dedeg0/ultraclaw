...
import type { UltraClawConfig, PluginRuntime } from "ultraclaw/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): UltraClawConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
