...
vi.mock("ultraclaw/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("ultraclaw/plugin-sdk/diagnostics-otel")>(
    "ultraclaw/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { UltraClawPluginServiceContext } from "ultraclaw/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "ultraclaw/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/ultraclaw-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): UltraClawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("ultraclaw.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("ultraclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("ultraclaw.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("ultraclaw.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("ultraclaw.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("ultraclaw.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("ultraclaw.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("ultraclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("ultraclaw.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("ultraclaw.webhook.processed");
    expect(spanNames).toContain("ultraclaw.message.processed");
    expect(spanNames).toContain("ultraclaw.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["ultraclaw.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("ultraclaw.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "ultraclaw.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["ultraclaw.reason"]).toBe("string");
    expect(String(attrs?.["ultraclaw.reason"])).not.toContain(
