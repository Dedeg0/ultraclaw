...
function resolveUltraClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.ULTRACLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set ULTRACLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveUltraClawRoot(), "dist", "extensionAPI.js");
