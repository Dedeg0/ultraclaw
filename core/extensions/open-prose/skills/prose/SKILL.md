...
metadata: { "ultraclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == UltraClaw `sessions_spawn`
- **File I/O** == UltraClaw `read`/`write`
- **Remote fetch** == UltraClaw `web_fetch` (or `exec` with curl when POST is required)
