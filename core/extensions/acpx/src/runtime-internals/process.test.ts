...
async function createTempDir(): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), "ultraclaw-acpx-process-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("spawnAndCollect", () => {
  ...
  it("strips shared provider auth env vars from spawned acpx children", async () => {
    ...
    vi.stubEnv("ULTRACLAW_API_KEY", "keep-me");

    const result = await spawnAndCollect({
      command: process.execPath,
      args: [
        "-e",
        "process.stdout.write(JSON.stringify({openai:process.env.OPENAI_API_KEY,github:process.env.GITHUB_TOKEN,hf:process.env.HF_TOKEN,ultraclaw:process.env.ULTRACLAW_API_KEY,shell:process.env.ULTRACLAW_SHELL}))",
      ],
      cwd: process.cwd(),
    ...
    expect(parsed.ultraclaw).toBe("keep-me");
  ...
  it("strips provider auth env vars case-insensitively", async () => {
    ...
    const result = await spawnAndCollect({
      ...
      args: [
        "-e",
        "process.stdout.write(JSON.stringify({openai:process.env.OpenAI_Api_Key,github:process.env.Github_Token,ultraclaw:process.env.ULTRACLAW_API_KEY,shell:process.env.ULTRACLAW_SHELL}))",
      ],
      cwd: process.cwd(),
      stripProviderAuthEnvVars: true,
    });
