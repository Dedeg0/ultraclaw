...
import type { UltraClawPluginApi, UltraClawPluginToolContext } from "ultraclaw/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<UltraClawPluginApi> = {}): UltraClawPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<UltraClawPluginToolContext> = {}): UltraClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: UltraClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
