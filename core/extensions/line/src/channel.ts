...
} from "ultraclaw/plugin-sdk/compat";
import {
...
} from "ultraclaw/plugin-sdk/line";
...
const lineConfigBase = createScopedChannelConfigBase<ResolvedLineAccount, UltraClawConfig>({
  sectionKey: "line",
...
const resolveLineDmPolicy = createScopedDmSecurityResolver<ResolvedLineAccount>({
  ...
  approveHint: "ultraclaw pairing approve line <code>",
...
export const linePlugin: ChannelPlugin<ResolvedLineAccount> = {
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      ...
      await line.pushMessageLine(id, "UltraClaw: your access has been approved.", {
  ...
  gateway: {
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      const nextCfg = { ...cfg } as UltraClawConfig;
      const lineConfig = (cfg.channels?.line ?? {}) as LineConfig;
