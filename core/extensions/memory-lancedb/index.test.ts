...
const liveEnabled = HAS_OPENAI_KEY && process.env.ULTRACLAW_LIVE_TEST === "1";
...
describe("memory plugin e2e", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-memory-test-"));
...
describeLive("memory plugin live tests", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-memory-live-"));
    dbPath = path.join(tmpDir, "lancedb");
  });
