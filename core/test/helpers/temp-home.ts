...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked ULTRACLAW_HOME.
  delete process.env.ULTRACLAW_HOME;
  process.env.ULTRACLAW_STATE_DIR = path.join(base, ".ultraclaw");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "ultraclaw-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".ultraclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
