...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `ultraclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".ultraclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          ULTRACLAW_SKIP_PROVIDERS: "1",
          ULTRACLAW_SKIP_GMAIL_WATCHER: "1",
          ULTRACLAW_SKIP_CRON: "1",
          ULTRACLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
          ULTRACLAW_SKIP_CANVAS_HOST: "1",
          ULTRACLAW_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
