    ...
    await withTempHome(
      async (tempHome) => {
        ...
        const env = {
          ...
          ULTRACLAW_TEST_FAST: "1",
        };
        delete env.ULTRACLAW_HOME;
        delete env.ULTRACLAW_STATE_DIR;
        delete env.ULTRACLAW_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "ultraclaw.mjs");
