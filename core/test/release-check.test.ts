        ...
        extensions: [
          {
            ...
            packageJson: {
              ...
              ultraclaw: {
                install: { npmSpec: "@ultraclaw/googlechat" },
                releaseChecks: {
          ...
          },
          {
            ...
            packageJson: {
              ...
              ultraclaw: { install: { npmSpec: "@ultraclaw/feishu" } },
            },
          },
        ],
      }),
...
describe("collectBundledExtensionManifestErrors", () => {
  it("flags invalid bundled extension install metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | ultraclaw.install.npmSpec must be a non-empty string",
    ]);
  });
  ...
  it("flags invalid release-check allowlist metadata", () => {
    expect(
      collectBundledExtensionManifestErrors([
        {
          ...
          packageJson: {
            ultraclaw: {
              install: { npmSpec: "@ultraclaw/broken" },
              releaseChecks: {
      ...
      "bundled extension 'broken' manifest invalid | ultraclaw.releaseChecks.rootDependencyMirrorAllowlist must contain only non-empty strings",
    ]);
  });
});
  ...
  it("flags nested node_modules leaking into npm pack output", () => {
    expect(
      collectForbiddenPackPaths([
        ...
        "node_modules/.bin/ultraclaw",
      ]),
    ).toEqual(["extensions/tlon/node_modules/.bin/tlon", "node_modules/.bin/ultraclaw"]);
  });
});
