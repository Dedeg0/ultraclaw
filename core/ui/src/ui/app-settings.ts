...
import type { UltraClawApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as UltraClawApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as UltraClawApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as UltraClawApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as UltraClawApp);
    await loadConfig(host as unknown as UltraClawApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as UltraClawApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as UltraClawApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as UltraClawApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as UltraClawApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as UltraClawApp);
    await loadDevices(host as unknown as UltraClawApp);
    await loadConfig(host as unknown as UltraClawApp);
    await loadExecApprovals(host as unknown as UltraClawApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as UltraClawApp);
    await loadConfig(host as unknown as UltraClawApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as UltraClawApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as UltraClawApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__ULTRACLAW_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as UltraClawApp;
...
async function loadOverviewLogs(host: UltraClawApp) {
...
function buildAttentionItems(host: UltraClawApp) {
  ...
  if (auth?.scopes && !hasOperatorReadAccess(auth)) {
    items.push({
      ...
      href: "https://docs.ultraclaw.os/web/dashboard",
      external: true,
    });
  }
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as UltraClawApp, true),
    loadConfigSchema(host as unknown as UltraClawApp),
    loadConfig(host as unknown as UltraClawApp),
  ]);
}
