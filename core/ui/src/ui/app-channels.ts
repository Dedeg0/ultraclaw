...
export async function handleWhatsAppStart(host: UltraClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: UltraClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: UltraClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: UltraClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: UltraClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: UltraClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: UltraClawApp): string | null {
...
function buildGatewayHttpHeaders(host: UltraClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: UltraClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: UltraClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: UltraClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: UltraClawApp) {
  const state = host.nostrProfileFormState;
