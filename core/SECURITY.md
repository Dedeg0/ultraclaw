...
If you believe you've found a security issue in UltraClaw, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [ultraclaw/ultraclaw](https://github.com/ultraclaw/ultraclaw)
- **macOS desktop app** — [ultraclaw/ultraclaw](https://github.com/ultraclaw/ultraclaw) (apps/macos)
- **iOS app** — [ultraclaw/ultraclaw](https://github.com/ultraclaw/ultraclaw) (apps/ios)
- **Android app** — [ultraclaw/ultraclaw](https://github.com/ultraclaw/ultraclaw) (apps/android)
- **ClawHub** — [ultraclaw/clawhub](https://github.com/ultraclaw/clawhub)
- **Trust and threat model** — [ultraclaw/trust](https://github.com/ultraclaw/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@ultraclaw.os](mailto:security@ultraclaw.os)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.ultraclaw.os).
...
- Tested version details (UltraClaw version and/or commit SHA).
...
- Demonstrated impact tied to UltraClaw's documented trust boundaries.
- For exposed-secret reports: proof the credential is UltraClaw-owned (or grants access to UltraClaw-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at UltraClaw. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

UltraClaw is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
UltraClaw does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- UltraClaw can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need UltraClaw, use one VPS (or host/OS user boundary) per user.
...
- This is expected in UltraClaw's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of UltraClaw's trusted computing base for a gateway.
...
- Using UltraClaw in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.ultraclaw`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not UltraClaw-owned and not granting access to UltraClaw-operated infrastructure/services) without demonstrated UltraClaw impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

UltraClaw security guidance assumes:

- The host where UltraClaw runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.ultraclaw` state/config (including `ultraclaw.json`) is effectively a trusted operator.
...
UltraClaw's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
UltraClaw separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when UltraClaw can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the UltraClaw process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

UltraClaw uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/ultraclaw` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/ultraclaw` (or `ultraclaw-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the UltraClaw-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use UltraClaw temp helpers (`resolvePreferredUltraClawTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-ultraclaw-dir.ts`
...
For threat model + hardening guidance (including `ultraclaw security audit --deep` and `--fix`), see:

- `https://docs.ultraclaw.os/gateway/security`
...
UltraClaw's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `ultraclaw gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - UltraClaw keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `ultraclaw security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__ultraclaw__/canvas/`, `/__ultraclaw__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__ultraclaw__/canvas/`, `/__ultraclaw__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

UltraClaw requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running UltraClaw in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v ultraclaw-data:/app/data \
  ultraclaw/ultraclaw:latest
```
