...
- Publish as `ultraclaw/plugin-sdk` (or export from core under `ultraclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `UltraClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: UltraClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: UltraClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: UltraClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: UltraClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: UltraClawConfig): string;
  };
};
```
...
- Introduce `ultraclaw/plugin-sdk`.
- Add `api.runtime` to `UltraClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `ultraclawRuntime: ">=2026.2.0"`).
