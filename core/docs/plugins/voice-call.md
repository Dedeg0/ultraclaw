...
read_when:
  - You want to place an outbound voice call from UltraClaw
...
Voice calls for UltraClaw via a plugin. Supports outbound notifications and
...
- Use `ultraclaw voicecall ...` or the `voice_call` tool
...
```bash
ultraclaw plugins install ./extensions/voice-call
