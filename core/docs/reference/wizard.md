...
This is the full reference for the `ultraclaw onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.ultraclaw/credentials/oauth.json`; auth profiles live in `~/.ultraclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.ultraclaw/credentials/oauth.json` (or `$ULTRACLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.ultraclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `ultraclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `ultraclaw configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `ultraclaw health`.
    - Tip: `ultraclaw status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export ULTRACLAW_GATEWAY_TOKEN="your-token"
ultraclaw onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env ULTRACLAW_GATEWAY_TOKEN
```
...
- Stores it under `~/.ultraclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.ultraclaw/ultraclaw.json`:
...
`ultraclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.ultraclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.ultraclaw/agents/<agentId>/sessions/`.
