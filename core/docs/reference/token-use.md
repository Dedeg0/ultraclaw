---
summary: "How UltraClaw builds prompt context and reports token usage + costs"
read_when:
...
UltraClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

UltraClaw assembles its own system prompt on every run. It includes:
...
For images, UltraClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from UltraClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `ultraclaw status --usage` and `ultraclaw channels list` show
...
`cacheWrite`. If pricing is missing, UltraClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. UltraClaw can
...
Anthropic's 1M context window is currently beta-gated. UltraClaw can inject the
...
UltraClaw skips the `context-1m-*` beta header because Anthropic currently
