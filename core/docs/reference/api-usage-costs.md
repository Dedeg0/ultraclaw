...
UltraClaw features that can generate provider usage or paid API calls.
...
- `ultraclaw status --usage` and `ultraclaw channels list` show provider **usage windows**
...
UltraClaw can pick up credentials from:
...
- `ultraclaw status --usage`
- `ultraclaw models status --json`
...
`ultraclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
