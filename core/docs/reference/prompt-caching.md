...
- With Anthropic API-key auth profiles, UltraClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, UltraClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
UltraClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.ultraclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$ULTRACLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `ULTRACLAW_CACHE_TRACE=1` enables cache tracing.
- `ULTRACLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `ULTRACLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `ULTRACLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `ULTRACLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
