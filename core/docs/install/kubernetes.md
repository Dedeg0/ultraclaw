---
summary: "Deploy UltraClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run UltraClaw on a Kubernetes cluster
  - You want to test UltraClaw in a Kubernetes environment
title: "Kubernetes"
---

# UltraClaw on Kubernetes

A minimal starting point for running UltraClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.
...
kubectl port-forward svc/ultraclaw 18790:18790 -n ultraclaw
...
```bash
kubectl get secret ultraclaw-secrets -n ultraclaw -o jsonpath='{.data.ULTRACLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: ultraclaw (configurable via ULTRACLAW_NAMESPACE)
├── Deployment/ultraclaw        # Single pod, init container + gateway
├── Service/ultraclaw           # ClusterIP on port 18790
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/ultraclaw-config  # ultraclaw.json + AGENTS.md
└── Secret/ultraclaw-secrets    # Gateway token + API keys
```
...
Edit `ultraclaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret ultraclaw-secrets -n ultraclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/ultraclaw -n ultraclaw
```

### Custom namespace

```bash
ULTRACLAW_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/ultraclaw/ultraclaw:2026.3.1
```
...
└── manifests/
    ...
    ├── configmap.yaml          # ultraclaw.json + AGENTS.md
