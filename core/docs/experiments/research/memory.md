...
read_when:
  - Designing workspace memory (~/.ultraclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep UltraClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.ultraclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.ultraclaw/workspace` as canonical human-readable memory.
...
```
~/.ultraclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in UltraClaw**, but keep a separable core library.

### Why integrate into UltraClaw?

- UltraClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `ultraclaw memory recall "…" --k 25 --since 30d`
  - `ultraclaw memory reflect --since 7d`
...
Pragmatic take for `~/.ultraclaw/workspace`:
