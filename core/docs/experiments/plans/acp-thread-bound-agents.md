...
This plan defines how UltraClaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- UltraClaw core owns ACP control plane concerns
...
UltraClaw should not reimplement ACP transport internals in core.
UltraClaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in UltraClaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under UltraClaw state dir
...
Core (must be in UltraClaw core):
...
- ACP event stream is normalized to UltraClaw reply chunks
...
For the first production backend (`extensions/acpx`), UltraClaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses UltraClaw `sessionKey`)
...
- UltraClaw consumes ndjson events from `acpx --format json --json-strict`
