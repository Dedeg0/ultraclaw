...
The onboarding wizard is the **recommended** way to set up UltraClaw on macOS,
...
`ultraclaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`ultraclaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.ultraclaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `ultraclaw doctor` first.
</Note>
...
Use `ultraclaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.ultraclaw/workspace-<agentId>`.
...
- CLI command reference: [`ultraclaw onboard`](/cli/onboard)
