---
summary: "Run UltraClaw with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run UltraClaw against a local SGLang server
...
UltraClaw can connect to SGLang using the `openai-completions` API.

UltraClaw can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, UltraClaw will query:
