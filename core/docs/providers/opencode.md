---
summary: "Use OpenCode Zen and Go catalogs with UltraClaw"
read_when:
...
OpenCode exposes two hosted catalogs in UltraClaw:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. UltraClaw keeps the runtime provider ids
...
ultraclaw onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
ultraclaw onboard --auth-choice opencode-go
ultraclaw onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
