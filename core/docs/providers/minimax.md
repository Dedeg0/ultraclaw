---
summary: "Use MiniMax M2.5 in UltraClaw"
read_when:
  - You want MiniMax models in UltraClaw
...
```bash
ultraclaw plugins enable minimax-portal-auth  # skip if already loaded.
ultraclaw gateway restart  # restart if gateway is already running
ultraclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/ultraclaw/ultraclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `ultraclaw.json`:
...
- Use `ultraclaw models list` and `ultraclaw models set minimax/MiniMax-M2.5` to switch.
...
- Running `ultraclaw configure` and selecting **MiniMax M2.5**, or
