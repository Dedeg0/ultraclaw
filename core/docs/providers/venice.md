---
summary: "Use Venice AI privacy-focused models in UltraClaw"
read_when:
  - You want privacy-focused inference in UltraClaw
...
```bash
ultraclaw onboard --auth-choice venice-api-key
```
...
```bash
ultraclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, UltraClaw shows all available Venice models. Pick based on your needs:
...
```bash
ultraclaw models set venice/kimi-k2-5
ultraclaw models set venice/claude-opus-4-6
```

List all available models:

```bash
ultraclaw models list | grep venice
```

## Configure via `ultraclaw configure`

1. Run `ultraclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
UltraClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
ultraclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
ultraclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
ultraclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
ultraclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
ultraclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `ultraclaw models list` to see currently available models. Some models may be temporarily offline.
