...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in UltraClaw
...
Running UltraClaw via Node with `tsx` fails at startup with:

```
[ultraclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `ultraclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch ultraclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node ultraclaw.mjs status` works on Node 25.
