...
- `/usage cost` in chats: local cost summary aggregated from UltraClaw session logs.
- CLI: `ultraclaw status --usage` prints a full per-provider breakdown.
- CLI: `ultraclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
