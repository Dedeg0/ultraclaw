---
summary: "CLI reference for `ultraclaw onboard` (interactive onboarding wizard)"
read_when:
...
```bash
ultraclaw onboard
ultraclaw onboard --flow quickstart
ultraclaw onboard --flow manual
ultraclaw onboard --mode remote --remote-url wss://gateway-host:18790
```

For plaintext private-network `ws://` targets (trusted networks only), set
`ULTRACLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `ultraclaw gateway run`.
...
- Fastest first chat: `ultraclaw dashboard` (Control UI, no channel setup).
