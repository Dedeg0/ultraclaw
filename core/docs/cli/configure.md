---
summary: "CLI reference for `ultraclaw configure` (interactive configuration prompts)"
read_when:
...
Tip: `ultraclaw config` without a subcommand opens the same wizard. Use
`ultraclaw config get|set|unset` for non-interactive edits.
...
```bash
ultraclaw configure
ultraclaw configure --section model --section channels
```
