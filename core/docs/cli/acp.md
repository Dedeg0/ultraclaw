...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a UltraClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`ultraclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the UltraClaw gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
ultraclaw acp --url wss://gateway-host:18790 --token <token>

# Remote Gateway (token from file)
ultraclaw acp --url wss://gateway-host:18790 --token-file ~/.ultraclaw/gateway.token

# Attach to an existing session key
ultraclaw acp --session agent:main:main

# Attach by label (must already exist)
ultraclaw acp --session-label "support inbox"

# Reset the session key before the first prompt
ultraclaw acp --session agent:main:main --reset-session
```
...
```bash
ultraclaw acp client

# Point the spawned bridge at a remote Gateway
ultraclaw acp client --server-args --url wss://gateway-host:18790 --token-file ~/.ultraclaw/gateway.token

# Override the server command (default: ultraclaw)
ultraclaw acp client --server "node" --server-args ultraclaw.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a UltraClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `ultraclaw acp` over stdio.

Example config (persisted):

```bash
ultraclaw config set gateway.remote.url wss://gateway-host:18790
ultraclaw config set gateway.remote.token <token>
```
...
```bash
ultraclaw acp --session agent:main:main
ultraclaw acp --session agent:design:main
ultraclaw acp --session agent:qa:bug-123
```
...
UltraClaw bot over ACP, use `acpx` with its built-in `ultraclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx ultraclaw` at `ultraclaw acp`.
3. Target the UltraClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default UltraClaw ACP session
acpx ultraclaw exec "Summarize the active UltraClaw session state."

# Persistent named session for follow-up turns
acpx ultraclaw sessions ensure --name codex-bridge
acpx ultraclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my UltraClaw work agent for recent context relevant to this repo."
```

If you want `acpx ultraclaw` to target a specific Gateway and session key every
time, override the `ultraclaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "ultraclaw": {
      "command": "env ULTRACLAW_HIDE_BANNER=1 ULTRACLAW_SUPPRESS_NOTES=1 ultraclaw acp --url ws://127.0.0.1:18790 --token-file ~/.ultraclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local UltraClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env ULTRACLAW_HIDE_BANNER=1 ULTRACLAW_SUPPRESS_NOTES=1 node ultraclaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an UltraClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select “UltraClaw ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`ULTRACLAW_GATEWAY_TOKEN`, `ULTRACLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`ULTRACLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `ULTRACLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `ultraclaw acp client` sets `ULTRACLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `ultraclaw`).
