...
read_when:
  - Setting up UltraClaw for the first time
...
UltraClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.ultraclaw/ultraclaw.json`.

If the file is missing, UltraClaw uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `ultraclaw onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    ultraclaw onboard       # full setup wizard
    ultraclaw configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Control UI">
    Open [http://127.0.0.1:18790](http://127.0.0.1:18790) and use the **Config** tab.
    The Control UI renders a form from the config schema, with a **Raw JSON** editor as an escape hatch.
  </Tab>
  <Tab title="Direct edit">
    Edit `~/.ultraclaw/ultraclaw.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
UltraClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`ultraclaw doctor`, `ultraclaw logs`, `ultraclaw health`, `ultraclaw status`)
- Run `ultraclaw doctor` to see exact issues
- Run `ultraclaw doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `ultraclaw.json`.
    ...
    ```bash
    ultraclaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `ULTRACLAW_APNS_RELAY_BASE_URL` and `ULTRACLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `ULTRACLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.ultraclaw/ultraclaw.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `ultraclaw config set` for single keys.
    </Warning>
    ...
    ultraclaw gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.ultraclaw/workspace\" } } }",
      "baseHash": "<hash>",
...
UltraClaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.ultraclaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, UltraClaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `ULTRACLAW_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${ULTRACLAW_GATEWAY_TOKEN}" } },
