...
read_when:
  - Running UltraClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of UltraClaw
...
- You run UltraClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. UltraClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. UltraClaw extracts the user identity from the configured header
...
- UltraClaw can stay on loopback HTTP behind the proxy.
...
If UltraClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://ultraclaw.example.com
    to: http://ultraclaw-gateway:18790
    policy:
      - allow:
          or:
            - email:
...
```
ultraclaw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy ultraclaw:18790 {
...
location / {
    ...
    proxy_pass http://ultraclaw:18790;
...
`ultraclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update UltraClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `ultraclaw security audit` and review findings
