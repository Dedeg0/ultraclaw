...
`ultraclaw doctor` is the repair + migration tool for UltraClaw. It fixes stale
...
- Extra workspace dir detection (`~/ultraclaw`).
...
you to run `ultraclaw doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.ultraclaw/ultraclaw.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.ultraclaw/sessions/` to `~/.ultraclaw/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.ultraclaw/agent/` to `~/.ultraclaw/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.ultraclaw/credentials/*.json` (except `oauth.json`)
  - to `~/.ultraclaw/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `ultraclaw doctor`.

### 3b) Legacy cron store migrations

Doctor also checks the cron job store (`~/.ultraclaw/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.ultraclaw` folders exist across
  home directories or when `ULTRACLAW_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.ultraclaw/ultraclaw.json` is
...
offers to remove them and install the UltraClaw service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named UltraClaw gateway services are considered first-class and are not
flagged as "extra."
...
- `ultraclaw doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `ultraclaw doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
- `ultraclaw doctor` prompts before rewriting supervisor config.
- `ultraclaw doctor --yes` accepts the default repair prompts.
- `ultraclaw doctor --repair` applies recommended fixes without prompts.
- `ultraclaw doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `ultraclaw gateway install --force`.
