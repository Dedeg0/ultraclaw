...
- During heartbeat runs, UltraClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `ultraclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, UltraClaw skips the heartbeat run entirely (no model call).
...
```bash
ultraclaw system event --text "Check for urgent follow-ups" --mode now
```
