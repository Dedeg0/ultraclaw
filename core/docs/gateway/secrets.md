...
UltraClaw supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `ULTRACLAW_GATEWAY_TOKEN` (or `CLAWDBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, UltraClaw runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). UltraClaw validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/UltraClaw QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/ultraclaw"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `ultraclaw.json` refs)
...
When reload-time activation fails after a healthy state, UltraClaw enters degraded secrets state.
...
- Strict command paths (for example `ultraclaw memory` remote-memory paths and `ultraclaw qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `ultraclaw status`, `ultraclaw status --all`, `ultraclaw channels status`, `ultraclaw channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `ultraclaw secrets reload`.
...
```bash
ultraclaw secrets audit --check
ultraclaw secrets configure
ultraclaw secrets audit --check
```
...
- plaintext values at rest (`ultraclaw.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `ultraclaw.json` refs)
...
- lets you select supported secret-bearing fields in `ultraclaw.json` plus `auth-profiles.json` for one agent scope
...
- `ultraclaw secrets configure --providers-only`
- `ultraclaw secrets configure --skip-provider-setup`
- `ultraclaw secrets configure --agent <id>`
...
```bash
ultraclaw secrets apply --from /tmp/ultraclaw-secrets-plan.json
ultraclaw secrets apply --from /tmp/ultraclaw-secrets-plan.json --dry-run
```
...
UltraClaw intentionally does not write rollback backups containing historical plaintext secret values.
