---
read_when:
  - 在 Raspberry Pi 上设置 UltraClaw 时
  - 在 ARM 设备上运行 UltraClaw 时
  - 构建低成本常驻个人 AI 时
summary: 在 Raspberry Pi 上运行 UltraClaw（低成本自托管设置）
title: Raspberry Pi
x-i18n:
...
# 在 Raspberry Pi 上运行 UltraClaw

## 目标

在 Raspberry Pi 上运行持久、常驻的 UltraClaw Gateway 网关，**一次性成本约 $35-80**（无月费）。
...
```bash
# 从你的笔记本电脑/台式机
ssh -L 18790:localhost:18790 user@gateway-host
...
大多数 UltraClaw 功能在 ARM64 上可用，但某些外部二进制文件可能需要 ARM 构建：
