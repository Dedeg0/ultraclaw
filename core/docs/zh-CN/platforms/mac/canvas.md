...
- `~/Library/Application Support/UltraClaw/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `ultraclaw-canvas://<session>/<path>`

示例：

- `ultraclaw-canvas://main/` → `<canvasRoot>/main/index.html`
- `ultraclaw-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `ultraclaw-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
ultraclaw nodes canvas present --node <id>
ultraclaw nodes canvas navigate --node <id> --url "/"
ultraclaw nodes canvas eval --node <id> --js "document.title"
ultraclaw nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__ultraclaw__/a2ui/
```
...
ultraclaw nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
ultraclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "ultraclaw://agent?message=Review%20this%20design";
```
