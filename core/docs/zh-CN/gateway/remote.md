...
- Gateway WebSocket 绑定到你配置端口的 **loopback**（默认为 18790）。
...
- 使用 macOS 应用的 **Remote over SSH** 模式（设置 → 通用 → "UltraClaw runs"）。
...
- `ultraclaw health` 和 `ultraclaw status --deep` 现在通过 `ws://127.0.0.1:18790` 访问远程 Gateway 网关。
- `ultraclaw gateway {status,health,send,agent,call}` 在需要时也可以通过 `--url` 指定转发的 URL。

注意：将 `18790` 替换为你配置的 `gateway.port`（或 `--port`/`ULTRACLAW_GATEWAY_PORT`）。
...
当 Gateway 网关仅限 loopback 时，保持 URL 为 `ws://127.0.0.1:18790` 并先打开 SSH 隧道。

## 通过 SSH 的聊天 UI

WebChat 不再使用单独的 HTTP 端口。SwiftUI 聊天 UI 直接连接到 Gateway WebSocket。

- 通过 SSH 转发 `18790`（见上文），然后让客户端连接到 `ws://127.0.0.1:18790`。
