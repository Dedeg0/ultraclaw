...
summary: 设置指南：在保持最新的同时保持你的 UltraClaw 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.ultraclaw/workspace`（工作区）+ `~/.ultraclaw/ultraclaw.json`（配置）。
...
- **配置：** `~/.ultraclaw/ultraclaw.json`（JSON/JSON5 格式）
- **工作区：** `~/.ultraclaw/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm ultraclaw setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **UltraClaw.app**（菜单栏）。
...
- 运行 `ultraclaw setup`，然后 `ultraclaw channels login`，然后手动启动 Gateway 网关（`ultraclaw gateway`）。
...
- **端口错误：** Gateway 网关 WS 默认为 `ws://127.0.0.1:18790`；保持应用 + CLI 在同一端口上。
- **状态存储位置：**
...
- **配对允许列表**：`~/.ultraclaw/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.ultraclaw/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.ultraclaw/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.ultraclaw/workspace` 和 `~/.ultraclaw/` 保持为"你的东西"；不要将个人提示/配置放入 `ultraclaw` 仓库。
...
- [UltraClaw 助手设置](/start/ultraclaw)
