---
read_when:
  - 你想在 UltraClaw 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 UltraClaw 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
ultraclaw plugins enable minimax-portal-auth  # 如果已加载则跳过
ultraclaw gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/ultraclaw/ultraclaw/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `ultraclaw models list` 和 `ultraclaw models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `ultraclaw configure` 并选择 **MiniMax M2.1**，或
