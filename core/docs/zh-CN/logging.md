...
你可以在 `~/.ultraclaw/ultraclaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.ultraclaw/ultraclaw.json` 的 `logging` 下。
...
- UltraClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `ultraclaw plugins enable diagnostics-otel` 启用插件。
...
- `ultraclaw.tokens`（计数器，属性：`ultraclaw.token`、`ultraclaw.channel`、`ultraclaw.provider`、`ultraclaw.model`）
- `ultraclaw.cost.usd`（计数器，属性：`ultraclaw.channel`、`ultraclaw.provider`、`ultraclaw.model`）
- `ultraclaw.run.duration_ms`（直方图，属性：`ultraclaw.channel`、`ultraclaw.provider`、`ultraclaw.model`）
- `ultraclaw.context.tokens`（直方图，属性：`ultraclaw.context`、`ultraclaw.channel`、`ultraclaw.provider`、`ultraclaw.model`）

消息流：

- `ultraclaw.webhook.received`（计数器，属性：`ultraclaw.channel`、`ultraclaw.webhook`）
- `ultraclaw.webhook.error`（计数器，属性：`ultraclaw.channel`、`ultraclaw.webhook`）
- `ultraclaw.webhook.duration_ms`（直方图，属性：`ultraclaw.channel`、`ultraclaw.webhook`）
- `ultraclaw.message.queued`（计数器，属性：`ultraclaw.channel`、`ultraclaw.source`）
- `ultraclaw.message.processed`（计数器，属性：`ultraclaw.channel`、`ultraclaw.outcome`）
- `ultraclaw.message.duration_ms`（直方图，属性：`ultraclaw.channel`、`ultraclaw.outcome`）

队列 + 会话：

- `ultraclaw.queue.lane.enqueue`（计数器，属性：`ultraclaw.lane`）
- `ultraclaw.queue.lane.dequeue`（计数器，属性：`ultraclaw.lane`）
- `ultraclaw.queue.depth`（直方图，属性：`ultraclaw.lane` 或 `ultraclaw.channel=heartbeat`）
- `ultraclaw.queue.wait_ms`（直方图，属性：`ultraclaw.lane`）
- `ultraclaw.session.state`（计数器，属性：`ultraclaw.state`、`ultraclaw.reason`）
- `ultraclaw.session.stuck`（计数器，属性：`ultraclaw.state`）
- `ultraclaw.session.stuck_age_ms`（直方图，属性：`ultraclaw.state`）
- `ultraclaw.run.attempt`（计数器，属性：`ultraclaw.attempt`）
...
- `ultraclaw.webhook.error`
  - `ultraclaw.channel`、`ultraclaw.webhook`、`ultraclaw.chatId`、`ultraclaw.error`
- `ultraclaw.message.processed`
  - `ultraclaw.channel`、`ultraclaw.outcome`、`ultraclaw.chatId`、`ultraclaw.messageId`、`ultraclaw.sessionKey`、`ultraclaw.sessionId`、`ultraclaw.reason`
- `ultraclaw.session.stuck`
  - `ultraclaw.state`、`ultraclaw.ageMs`、`ultraclaw.queueDepth`、`ultraclaw.sessionKey`、`ultraclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `ultraclaw doctor`。
