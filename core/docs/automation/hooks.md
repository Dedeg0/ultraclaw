...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in UltraClaw.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in UltraClaw. See [Webhook Hooks](/automation/webhook) or use `ultraclaw webhooks` for Gmail helper commands.
...
- Extend UltraClaw's behavior without modifying core code

## Getting Started

### Bundled Hooks

UltraClaw ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.ultraclaw/workspace/memory/`) when you issue `/new`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.ultraclaw/logs/commands.log`
...
During onboarding (`ultraclaw onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
2. **Managed hooks**: `~/.ultraclaw/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<ultraclaw>/dist/hooks/bundled/` (shipped with UltraClaw)
...
Hook packs are standard npm packages that export one or more hooks via `ultraclaw.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.ultraclaw/hooks/<id>`.
Each `ultraclaw.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `ultraclaw hooks install` installs dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.ultraclaw.os/automation/hooks#my-hook
metadata:
  { "ultraclaw": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
The `metadata.ultraclaw` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before UltraClaw persists them.
...
- **Managed hooks** (`~/.ultraclaw/hooks/`): Shared across workspaces

### 2. Create Directory Structure

```bash
mkdir -p ~/.ultraclaw/hooks/my-hook
cd ~/.ultraclaw/hooks/my-hook
```
...
metadata: { "ultraclaw": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# Enable it
ultraclaw hooks enable my-hook
...
# JSON output
ultraclaw hooks info session-memory --json
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.ultraclaw/workspace`)
...
```yaml
metadata: { "ultraclaw": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "ultraclaw": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.ultraclaw/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.ultraclaw/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.ultraclaw/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/ultraclaw/ultraclaw/tree/main/src/hooks/bundled)
