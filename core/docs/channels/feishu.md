...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects UltraClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current UltraClaw releases, so no separate plugin install
is required.
...
```bash
ultraclaw plugins install @ultraclaw/feishu
```
...
If you just installed UltraClaw, run the wizard:
...
2. Configuring app credentials in UltraClaw
...
1. You already ran `ultraclaw channels add` for Feishu
2. The gateway is running (`ultraclaw gateway status`)
...
```bash
ultraclaw pairing approve feishu <CODE>
```
...
2. Run `ultraclaw logs --follow` and look for `chat_id`
...
2. Run `ultraclaw logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `ultraclaw gateway status`  | Show gateway status           |
| `ultraclaw gateway install` | Install/start gateway service |
| `ultraclaw gateway stop`    | Stop gateway service          |
| `ultraclaw gateway restart` | Restart gateway service       |
| `ultraclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `ultraclaw logs --follow`
...
5. Ensure the gateway is running: `ultraclaw gateway status`
6. Check logs: `ultraclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.ultraclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.ultraclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
