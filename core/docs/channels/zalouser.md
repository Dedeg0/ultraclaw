...
read_when:
  - Setting up Zalo Personal for UltraClaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside UltraClaw.
...
- Install via CLI: `ultraclaw plugins install @ultraclaw/zalouser`
- Or from a source checkout: `ultraclaw plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `ultraclaw channels login --channel zalouser`
...
```bash
ultraclaw directory self --channel zalouser
ultraclaw directory peers list --channel zalouser --query "name"
ultraclaw directory groups list --channel zalouser --query "work"
```
...
- `ultraclaw pairing list zalouser`
- `ultraclaw pairing approve zalouser <code>`
...
- On startup, UltraClaw resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, UltraClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in UltraClaw state. Example:
...
- UltraClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, UltraClaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `ultraclaw channels logout --channel zalouser && ultraclaw channels login --channel zalouser`
...
- The channel now runs fully in UltraClaw without external CLI binaries.
