...
Matrix is an open, decentralized messaging protocol. UltraClaw connects as a Matrix **user**
...
```bash
ultraclaw plugins install @ultraclaw/matrix
```

Local checkout (when running from a git repo):

```bash
ultraclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `ultraclaw plugins install @ultraclaw/matrix`
   - From a local checkout: `ultraclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: UltraClaw calls the same
     login endpoint, stores the access token in `~/.ultraclaw/credentials/matrix/credentials.json`,
...
- On first connection, UltraClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  UltraClaw logs a warning.
...
`~/.ultraclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `ultraclaw pairing list matrix`
  - `ultraclaw pairing approve matrix <CODE>`
...
- On startup, UltraClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
