...
read_when:
  - Setting up Twitch chat integration for UltraClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. UltraClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
ultraclaw plugins install @ultraclaw/twitch
```

Local checkout (when running from a git repo):

```bash
ultraclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `ULTRACLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "ultraclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ULTRACLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
ULTRACLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
