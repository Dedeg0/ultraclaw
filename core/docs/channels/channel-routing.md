...
UltraClaw routes replies **back to the channel where a message came from**. The
...
UltraClaw infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, UltraClaw still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when UltraClaw would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.ultraclaw/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.ultraclaw`):

- `~/.ultraclaw/agents/<agentId>/sessions/sessions.json`
