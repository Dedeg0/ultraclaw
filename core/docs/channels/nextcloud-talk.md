...
```bash
ultraclaw plugins install @ultraclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
ultraclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "UltraClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `ultraclaw pairing list nextcloud-talk`
  - `ultraclaw pairing approve nextcloud-talk <CODE>`
