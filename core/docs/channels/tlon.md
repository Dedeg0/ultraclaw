...
Tlon is a decentralized messenger built on Urbit. UltraClaw connects to your Urbit ship and can
...
```bash
ultraclaw plugins install @ultraclaw/tlon
```

Local checkout (when running from a git repo):

```bash
ultraclaw plugins install ./extensions/tlon
```
...
By default, UltraClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `ultraclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, UltraClaw replies in-thread.
