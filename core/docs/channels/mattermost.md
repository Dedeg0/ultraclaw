---
summary: "Mattermost bot setup and UltraClaw config"
read_when:
...
```bash
ultraclaw plugins install @ultraclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
ultraclaw plugins install ./extensions/mattermost
```
...
4. Configure UltraClaw and start the gateway.
...
Native slash commands are opt-in. When enabled, UltraClaw registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, UltraClaw derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as UltraClaw.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to UltraClaw.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from UltraClaw, not `404`.
...
- Approve via:
  - `ultraclaw pairing list mattermost`
  - `ultraclaw pairing approve mattermost <CODE>`
...
Use these target formats with `ultraclaw message send` or cron/webhooks:
...
UltraClaw resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), UltraClaw sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
- If `interactions.callbackBaseUrl` is omitted, UltraClaw derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and UltraClaw run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="ultraclaw-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"ultraclaw-mattermost-interactions",
...
`ultraclaw message send` and cron/webhook deliveries.
