---
summary: "Chrome extension: let UltraClaw drive your existing Chrome tab"
read_when:
...
The UltraClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate ultraclaw-managed Chrome profile.

Attach/detach happens via a **single Chrome toolbar button**.

If you want Chrome’s official DevTools MCP attach flow instead of the UltraClaw
...
UltraClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the UltraClaw release (npm package) as static files. There is no separate “build” step.

After upgrading UltraClaw:

- Re-run `ultraclaw browser extension install` to refresh the installed files under your UltraClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

UltraClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `ULTRACLAW_GATEWAY_TOKEN`)
...
- Open the tab you want UltraClaw to control.
...
- `ON`: attached; UltraClaw can drive that tab.
...
Debugging: `ultraclaw sandbox explain`
...
`ultraclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `ultraclaw browser extension install` first to copy the extension to a stable location under your UltraClaw state directory.
...
- **This is not isolated** like the dedicated ultraclaw-managed profile.
