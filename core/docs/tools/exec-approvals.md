...
- **gateway host** → `ultraclaw` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, UltraClaw also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.ultraclaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.ultraclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `ultraclaw approvals allowlist ...`).
- `ultraclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `ultraclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.ultraclaw/exec-approvals.json` directly.

CLI: `ultraclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, UltraClaw does not treat
...
topics, UltraClaw preserves the topic for the approval prompt and the post-approval follow-up.
