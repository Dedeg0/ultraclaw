...
read_when:
  - Running UltraClaw Gateway in WSL2 while Chrome lives on Windows
...
- UltraClaw Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/ultraclaw/ultraclaw/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome` profile plus the UltraClaw Chrome extension.
...
- UltraClaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, UltraClaw is not the problem yet.
...
Fix that before touching UltraClaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point UltraClaw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting UltraClaw to succeed
...
```bash
ultraclaw browser open https://example.com --browser-profile remote
ultraclaw browser tabs --browser-profile remote
```

For the extension relay:

```bash
ultraclaw browser tabs --browser-profile chrome
```

Good result:

- the tab opens in Windows Chrome
- `ultraclaw browser tabs` returns the target
...
3. UltraClaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug UltraClaw config or Control UI auth
