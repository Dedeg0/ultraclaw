---
summary: "UltraClaw plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends UltraClaw with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core UltraClaw yet (or you want to keep optional features out of your main
install).
...
```bash
ultraclaw plugins list
```

2. Install an official plugin (example: Voice Call):

```bash
ultraclaw plugins install @ultraclaw/voice-call
```
...
UltraClaw's plugin system has four layers:

1. **Manifest + discovery**
   UltraClaw finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads
   `ultraclaw.plugin.json` plus package metadata first.
...
4. **Surface consumption**
   The rest of UltraClaw reads the registry to expose tools, channels, provider
...
That split lets UltraClaw validate config, explain missing/disabled plugins, and
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@ultraclaw/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@ultraclaw/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@ultraclaw/zalouser`
- [Matrix](/channels/matrix) — `@ultraclaw/matrix`
- [Nostr](/channels/nostr) — `@ultraclaw/nostr`
- [Zalo](/channels/zalo) — `@ultraclaw/zalo`
- [Microsoft Teams](/channels/msteams) — `@ultraclaw/msteams`
- Google Antigravity OAuth (provider auth) — bundled as `google-antigravity-auth` (disabled by default)
- Gemini CLI OAuth (provider auth) — bundled as `google-gemini-cli-auth` (disabled by default)
- Qwen OAuth (provider auth) — bundled as `qwen-portal-auth` (disabled by default)
- Copilot Proxy (provider auth) — local VS Code Copilot Proxy bridge; distinct from built-in `github-copilot` device login (bundled, disabled by default)

UltraClaw plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
At startup, UltraClaw does roughly this:

1. discover candidate plugin roots
2. read `ultraclaw.plugin.json` and package metadata
...
The manifest is the control-plane source of truth. UltraClaw uses it to:
...
UltraClaw keeps short in-process caches for:
...
Use SDK subpaths instead of the monolithic `ultraclaw/plugin-sdk` import when
authoring plugins:

- `ultraclaw/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `ultraclaw/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `ultraclaw/plugin-sdk/telegram` for Telegram channel plugins.
- `ultraclaw/plugin-sdk/discord` for Discord channel plugins.
- `ultraclaw/plugin-sdk/slack` for Slack channel plugins.
- `ultraclaw/plugin-sdk/signal` for Signal channel plugins.
- `ultraclaw/plugin-sdk/imessage` for iMessage channel plugins.
- `ultraclaw/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `ultraclaw/plugin-sdk/line` for LINE channel plugins.
- `ultraclaw/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `ultraclaw/plugin-sdk/acpx`, `ultraclaw/plugin-sdk/bluebubbles`,
  `ultraclaw/plugin-sdk/copilot-proxy`, `ultraclaw/plugin-sdk/device-pair`,
  `ultraclaw/plugin-sdk/diagnostics-otel`, `ultraclaw/plugin-sdk/diffs`,
  `ultraclaw/plugin-sdk/feishu`,
  `ultraclaw/plugin-sdk/google-gemini-cli-auth`, `ultraclaw/plugin-sdk/googlechat`,
  `ultraclaw/plugin-sdk/irc`, `ultraclaw/plugin-sdk/llm-task`,
  `ultraclaw/plugin-sdk/lobster`, `ultraclaw/plugin-sdk/matrix`,
  `ultraclaw/plugin-sdk/mattermost`, `ultraclaw/plugin-sdk/memory-core`,
  `ultraclaw/plugin-sdk/memory-lancedb`,
  `ultraclaw/plugin-sdk/minimax-portal-auth`,
  `ultraclaw/plugin-sdk/nextcloud-talk`, `ultraclaw/plugin-sdk/nostr`,
  `ultraclaw/plugin-sdk/open-prose`, `ultraclaw/plugin-sdk/phone-control`,
  `ultraclaw/plugin-sdk/qwen-portal-auth`, `ultraclaw/plugin-sdk/synology-chat`,
  `ultraclaw/plugin-sdk/talk-voice`, `ultraclaw/plugin-sdk/test-utils`,
  `ultraclaw/plugin-sdk/thread-ownership`, `ultraclaw/plugin-sdk/tlon`,
  `ultraclaw/plugin-sdk/twitch`, `ultraclaw/plugin-sdk/voice-call`,
  `ultraclaw/plugin-sdk/zalo`, and `ultraclaw/plugin-sdk/zalouser`.

Compatibility note:

- `ultraclaw/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `ultraclaw status`, `ultraclaw status --all`,
  `ultraclaw channels status`, `ultraclaw channels resolve`, and doctor/config
...
- Set `ULTRACLAW_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `ULTRACLAW_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `ULTRACLAW_PLUGIN_DISCOVERY_CACHE_MS` and
  `ULTRACLAW_PLUGIN_MANIFEST_CACHE_MS`.

## Discovery & precedence

UltraClaw scans, in order:
...
- `<workspace>/.ultraclaw/extensions/*.ts`
- `<workspace>/.ultraclaw/extensions/*/index.ts`

3. Global extensions

- `~/.ultraclaw/extensions/*.ts`
- `~/.ultraclaw/extensions/*/index.ts`

4. Bundled extensions (shipped with UltraClaw, mostly disabled by default)

- `<ultraclaw>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `ultraclaw plugins enable <id>`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, UltraClaw logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. UltraClaw blocks candidates when:
...
Each plugin must include a `ultraclaw.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `ultraclaw.extensions`:
...
Security guardrail: every `ultraclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `ultraclaw plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `ultraclaw.channel` and
install hints via `ultraclaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@ultraclaw/nextcloud-talk",
  "ultraclaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@ultraclaw/nextcloud-talk",
...
UltraClaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.ultraclaw/mpm/plugins.json`
- `~/.ultraclaw/mpm/catalog.json`
- `~/.ultraclaw/plugins/catalog.json`

Or point `ULTRACLAW_PLUGIN_CATALOG_PATHS` (or `ULTRACLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "ultraclaw": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, UltraClaw uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `ultraclaw.plugin.json` (`configSchema`).
...
UltraClaw preserves config for disabled plugins so toggling them back on is not
destructive.
...
UltraClaw augments `uiHints` at runtime based on discovered plugins:
...
```bash
ultraclaw plugins list
ultraclaw plugins info <id>
ultraclaw plugins install <path>                 # copy a local file/dir into ~/.ultraclaw/extensions/<id>
ultraclaw plugins install ./extensions/voice-call # relative path ok
ultraclaw plugins install ./plugin.tgz           # install from a local tarball
ultraclaw plugins install ./plugin.zip           # install from a local zip
ultraclaw plugins install -l ./extensions/voice-call # link (no copy) for dev
ultraclaw plugins install @ultraclaw/voice-call # install from npm
ultraclaw plugins install @ultraclaw/voice-call --pin # store exact resolved name@version
ultraclaw plugins update <id>
ultraclaw plugins update --all
ultraclaw plugins enable <id>
ultraclaw plugins disable <id>
ultraclaw plugins doctor
```

`plugins update` only works for npm installs tracked under `plugins.installs`.
If stored integrity metadata changes between updates, UltraClaw warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `ultraclaw voicecall`).
...
- Plugin-managed hooks show up in `ultraclaw hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `ultraclaw hooks`; enable/disable the plugin instead.
...
- When disabled, UltraClaw blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
setup inside UltraClaw, surface provider setup in onboarding/model-pickers, and
...
2. **Non-interactive setup**
   `auth[].runNonInteractive(ctx)` handles `ultraclaw onboard --non-interactive`
   without prompts. Use this when the provider needs custom headless setup
   beyond the built-in simple API-key paths.
3. **Wizard integration**
   `wizard.onboarding` adds an entry to `ultraclaw onboard`.
...
- `configPatch`: optional `ultraclaw.json` changes
...
explicit method or let UltraClaw synthesize per-method choices.

UltraClaw validates provider wizard metadata when the plugin registers:
...
- `ultraclaw models auth login --provider <id> [--method <id>]`
- `ultraclaw onboard`
...
- `wizard.onboarding` adds a provider choice to `ultraclaw onboard`.
...
- `config`: The current UltraClaw config
...
- Main package: `ultraclaw` (this repo)
- Plugins: separate npm packages under `@ultraclaw/*` (example: `@ultraclaw/voice-call`)

Publishing contract:

- Plugin `package.json` must include `ultraclaw.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `ultraclaw plugins install <npm-spec>` uses `npm pack`, extracts into `~/.ultraclaw/extensions/<id>/`, and enables it in config.
...
- CLI: `ultraclaw voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `ultraclaw.extensions` points at the built entrypoint (`dist/index.js`).
