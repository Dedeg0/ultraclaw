...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `ultraclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `ultraclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=ultraclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${ULTRACLAW_STATE_DIR:-$HOME/.ultraclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
