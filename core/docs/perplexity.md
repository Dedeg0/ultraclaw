...
UltraClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, UltraClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `ultraclaw configure --section web`. It stores the key in
`~/.ultraclaw/ultraclaw.json` under `tools.web.search.perplexity.apiKey`.
...
`~/.ultraclaw/.env` (or your service environment). See [Env vars](/help/faq#how-does-ultraclaw-load-environment-variables).
