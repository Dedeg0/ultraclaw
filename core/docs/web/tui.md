...
```bash
ultraclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `ultraclaw tui --deliver`
...
- Local shell commands receive `ULTRACLAW_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `ULTRACLAW_THEME=light` before launching `ultraclaw tui`.
- To force the original dark palette instead, set `ULTRACLAW_THEME=dark`.
...
- Check the Gateway logs: `ultraclaw logs --follow`.
- Confirm the agent can run: `ultraclaw status` and `ultraclaw models status`.
...
- No agents in picker: check `ultraclaw agents list` and your routing config.
