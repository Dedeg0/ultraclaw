...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/ultraclaw`)
...
If the page fails to load, start the Gateway first: `ultraclaw gateway`.
...
you revoke it with `ultraclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.ultraclaw/ultraclaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop ultraclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. UltraClaw
...
UltraClaw **blocks** Control UI connections without device identity.
