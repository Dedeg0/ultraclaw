...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `ultraclaw.internal.`), **or**
...
ultraclaw gateway call node.list --params "{}"
```
...
- `ULTRACLAW_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine UltraClaw iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight UltraClaw builds.
...
```bash
export ULTRACLAW_APNS_TEAM_ID="TEAMID"
export ULTRACLAW_APNS_KEY_ID="KEYID"
export ULTRACLAW_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_ultraclaw-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `ultraclaw.internal.`) and Tailscale split DNS.
...
```bash
ultraclaw nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18790/__ultraclaw__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__ultraclaw__/canvas/` and `/__ultraclaw__/a2ui/`.
...
```bash
ultraclaw nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__ultraclaw; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
ultraclaw nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `ultraclaw devices list` and approve manually.
