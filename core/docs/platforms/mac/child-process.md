...
service via the external `ultraclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.ultraclaw.gateway`
  (or `ai.ultraclaw.<profile>` when using `--profile`/`ULTRACLAW_PROFILE`; legacy `com.ultraclaw.*` is supported).
