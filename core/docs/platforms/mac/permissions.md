...
- Same path: run the app from a fixed location (for UltraClaw, `dist/UltraClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.ultraclaw.mac
sudo tccutil reset ScreenCapture ai.ultraclaw.mac
...
Workaround: move files into the UltraClaw workspace (`~/.ultraclaw/workspace`) if you want to avoid per-folder grants.
