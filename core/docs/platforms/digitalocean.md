---
summary: "UltraClaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up UltraClaw on DigitalOcean
  - Looking for cheap VPS hosting for UltraClaw
title: "DigitalOcean"
---

# UltraClaw on DigitalOcean

## Goal

Run a persistent UltraClaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status ultraclaw-gateway.service

# View logs
journalctl --user -u ultraclaw-gateway.service -f
```
...
```bash
ultraclaw config set gateway.bind tailnet
ultraclaw gateway restart
```
...
```bash
ultraclaw channels login whatsapp
# Scan QR code
```
...
- `~/.ultraclaw/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u ultraclaw --no-pager -n 50
```
