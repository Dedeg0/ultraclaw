---
summary: "UltraClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up UltraClaw on a Raspberry Pi
  - Running UltraClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# UltraClaw on Raspberry Pi

## Goal

Run a persistent, always-on UltraClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u ultraclaw -f
```

## 9) Access the UltraClaw Dashboard
...
```bash
ssh user@gateway-host 'ultraclaw dashboard --no-open'
```

The command prints `Dashboard URL:`. Depending on how `gateway.auth.token`
is configured, the URL may be a plain `http://127.0.0.1:18790/` link or one
that includes `#token=...`.
...
(or `ULTRACLAW_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/ultraclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/ultraclaw-compile-cache
mkdir -p /var/tmp/ultraclaw-compile-cache
export ULTRACLAW_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `ULTRACLAW_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running UltraClaw, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit ultraclaw
```

```ini
[Service]
Environment=ULTRACLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/ultraclaw-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep UltraClaw state/cache on SSD-backed storage to avoid SD-card
...
Most UltraClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled ultraclaw

# Enable if not
sudo systemctl enable ultraclaw

# Start on boot
sudo systemctl start ultraclaw
```
...
```bash
# Check logs
journalctl -u ultraclaw --no-pager -n 100

# Common fix: rebuild
cd ~/ultraclaw  # if using hackable install
npm run build
sudo systemctl restart ultraclaw
```
