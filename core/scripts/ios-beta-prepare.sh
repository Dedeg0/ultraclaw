...
- regenerates apps/ios/UltraClaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${ULTRACLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid ULTRACLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid ULTRACLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid ULTRACLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid ULTRACLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing ULTRACLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(ULTRACLAW_URL_SLASH)$(ULTRACLAW_URL_SLASH)#g'
)"
...
ULTRACLAW_CODE_SIGN_STYLE = Automatic
ULTRACLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
ULTRACLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
ULTRACLAW_APP_BUNDLE_ID = ai.ultraclaw.client
ULTRACLAW_SHARE_BUNDLE_ID = ai.ultraclaw.client.share
ULTRACLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.ultraclaw.client.activitywidget
ULTRACLAW_WATCH_APP_BUNDLE_ID = ai.ultraclaw.client.watchkitapp
ULTRACLAW_WATCH_EXTENSION_BUNDLE_ID = ai.ultraclaw.client.watchkitapp.extension
ULTRACLAW_APP_PROFILE =
ULTRACLAW_SHARE_PROFILE =
ULTRACLAW_PUSH_TRANSPORT = relay
ULTRACLAW_PUSH_DISTRIBUTION = official
ULTRACLAW_URL_SLASH = /
ULTRACLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
ULTRACLAW_PUSH_APNS_ENVIRONMENT = production
EOF
