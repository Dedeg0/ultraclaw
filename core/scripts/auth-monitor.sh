...
#
# Suggested cron: */30 * * * * /home/admin/ultraclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send UltraClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., ultraclaw-alerts)
...
STATE_FILE="$HOME/.ultraclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via UltraClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use ultraclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via UltraClaw to $NOTIFY_PHONE..."
            ultraclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! UltraClaw is down. Run: ssh l36 '~/ultraclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
