...
# Build and bundle UltraClaw into a minimal .app we can open.
# Outputs to dist/UltraClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/UltraClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="UltraClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.ultraclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/ultraclaw/ultraclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/UltraClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :UltraClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :UltraClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/UltraClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/UltraClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/UltraClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/UltraClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/UltraClaw/Resources/UltraClaw.icns" "$APP_ROOT/Contents/Resources/UltraClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/UltraClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying UltraClawKit resources"
ULTRACLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/UltraClawKit_UltraClawKit.bundle"
if [ -d "$ULTRACLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/UltraClawKit_UltraClawKit.bundle"
  cp -R "$ULTRACLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/UltraClawKit_UltraClawKit.bundle"
else
  echo "WARN: UltraClawKit resource bundle not found at $ULTRACLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running UltraClaw"
killall -q UltraClaw 2>/dev/null || true
