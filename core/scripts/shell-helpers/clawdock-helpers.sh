#!/usr/bin/env bash
# ClawDock - Docker helpers for UltraClaw
# Inspired by Simon Willison's "Running UltraClaw in Docker"
# https://til.simonwillison.net/llms/ultraclaw-docker
#
# Installation:
...
# Common paths to check for UltraClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/ultraclaw"
  "${HOME}/workspace/ultraclaw"
  "${HOME}/projects/ultraclaw"
  "${HOME}/dev/ultraclaw"
  "${HOME}/code/ultraclaw"
  "${HOME}/src/ultraclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found UltraClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/ultraclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ UltraClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/ultraclaw/ultraclaw.git ~/ultraclaw"
    echo "  cd ~/ultraclaw && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^ULTRACLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d ultraclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart ultraclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f ultraclaw-gateway
}
...
clawdock-workspace() {
  cd ~/.ultraclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec ultraclaw-gateway \
    bash -c 'echo "alias ultraclaw=\"./ultraclaw.mjs\"" > /tmp/.bashrc_ultraclaw && bash --rcfile /tmp/.bashrc_ultraclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm ultraclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build ultraclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "ULTRACLAW_GATEWAY_TOKEN=$token" ultraclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" ultraclaw-gateway \
    bash -c './ultraclaw.mjs config set gateway.remote.token "$TOKEN" && ./ultraclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec ultraclaw-gateway \
    bash -c "./ultraclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart ultraclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm ultraclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec ultraclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'ultraclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for UltraClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (ultraclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to ultraclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.ultraclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'ultraclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'ultraclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.ultraclaw.os${_CLR_RESET}"
  echo ""
}
