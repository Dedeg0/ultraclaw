...
const EXPECTED_REPOSITORY_URL = "https://github.com/ultraclaw/ultraclaw";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "ultraclaw") {
    errors.push(`package.json name must be "ultraclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.ultraclaw !== "ultraclaw.mjs") {
    errors.push(
      `package.json bin.ultraclaw must be "ultraclaw.mjs"; found "${pkg.bin?.ultraclaw ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`ultraclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `ultraclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
