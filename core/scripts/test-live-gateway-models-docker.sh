...
docker run --rm -t \
  ...
  -e ULTRACLAW_LIVE_GATEWAY_MODELS="${ULTRACLAW_LIVE_GATEWAY_MODELS:-${CLAWDBOT_LIVE_GATEWAY_MODELS:-modern}}" \
  -e ULTRACLAW_LIVE_GATEWAY_PROVIDERS="${ULTRACLAW_LIVE_GATEWAY_PROVIDERS:-${CLAWDBOT_LIVE_GATEWAY_PROVIDERS:-}}" \
  -e ULTRACLAW_LIVE_GATEWAY_MAX_MODELS="${ULTRACLAW_LIVE_GATEWAY_MAX_MODELS:-${CLAWDBOT_LIVE_GATEWAY_MAX_MODELS:-24}}" \
  -e ULTRACLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${ULTRACLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-${CLAWDBOT_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-}}" \
  -v "$ROOT_DIR":/src:ro \
