...
BASE_IMAGE="${BASE_IMAGE:-ultraclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-ultraclaw-sandbox-common:bookworm-slim}"
...
ULTRACLAW_DOCKER_BUILD_USE_BUILDX="${ULTRACLAW_DOCKER_BUILD_USE_BUILDX:-0}"
ULTRACLAW_DOCKER_BUILD_CACHE_FROM="${ULTRACLAW_DOCKER_BUILD_CACHE_FROM:-}"
ULTRACLAW_DOCKER_BUILD_CACHE_TO="${ULTRACLAW_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${ULTRACLAW_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${ULTRACLAW_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${ULTRACLAW_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${ULTRACLAW_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${ULTRACLAW_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=ultraclaw.sandbox=1)
NOTE
