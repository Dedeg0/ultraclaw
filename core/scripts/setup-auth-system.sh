#!/bin/bash
# Setup UltraClaw Auth Management System
...
echo "=== UltraClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'ultraclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  UltraClaw message: Send warning via UltraClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/ultraclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/ultraclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/ultraclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now ultraclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status ultraclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
