...
const shouldBuild = (deps) => {
  if (deps.env.ULTRACLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.ULTRACLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[ultraclaw] ${message}\n`);
};

const runUltraClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["ultraclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runUltraClaw(deps);
  }
