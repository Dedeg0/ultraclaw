...
# UltraClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://ultraclaw.os/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one UltraClaw."
...
GUM_VERSION="${ULTRACLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 UltraClaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://ultraclaw.os/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$ULTRACLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.ultraclaw.os/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_ultraclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.ultraclaw-* "$npm_root"/ultraclaw 2>/dev/null || true
}

extract_ultraclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_ultraclaw_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/ultraclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/ultraclaw"|"/usr/local/bin/ultraclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/ultraclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale ultraclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing ultraclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing UltraClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_ultraclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "UltraClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*ultraclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_ultraclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_ultraclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "UltraClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an ultraclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
map_legacy_env "ULTRACLAW_TAGLINE_INDEX" "CLAWDBOT_TAGLINE_INDEX"
map_legacy_env "ULTRACLAW_NO_ONBOARD" "CLAWDBOT_NO_ONBOARD"
map_legacy_env "ULTRACLAW_NO_PROMPT" "CLAWDBOT_NO_PROMPT"
map_legacy_env "ULTRACLAW_DRY_RUN" "CLAWDBOT_DRY_RUN"
map_legacy_env "ULTRACLAW_INSTALL_METHOD" "CLAWDBOT_INSTALL_METHOD"
map_legacy_env "ULTRACLAW_VERSION" "CLAWDBOT_VERSION"
map_legacy_env "ULTRACLAW_BETA" "CLAWDBOT_BETA"
map_legacy_env "ULTRACLAW_GIT_DIR" "CLAWDBOT_GIT_DIR"
map_legacy_env "ULTRACLAW_GIT_UPDATE" "CLAWDBOT_GIT_UPDATE"
map_legacy_env "ULTRACLAW_NPM_LOGLEVEL" "CLAWDBOT_NPM_LOGLEVEL"
map_legacy_env "ULTRACLAW_VERBOSE" "CLAWDBOT_VERBOSE"
map_legacy_env "ULTRACLAW_PROFILE" "CLAWDBOT_PROFILE"
map_legacy_env "ULTRACLAW_INSTALL_SH_NO_RUN" "CLAWDBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${ULTRACLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${ULTRACLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((ULTRACLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${ULTRACLAW_NO_ONBOARD:-0}
NO_PROMPT=${ULTRACLAW_NO_PROMPT:-0}
DRY_RUN=${ULTRACLAW_DRY_RUN:-0}
INSTALL_METHOD=${ULTRACLAW_INSTALL_METHOD:-}
ULTRACLAW_VERSION=${ULTRACLAW_VERSION:-latest}
USE_BETA=${ULTRACLAW_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/ultraclaw"
GIT_DIR=${ULTRACLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${ULTRACLAW_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${ULTRACLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${ULTRACLAW_VERBOSE:-0}"
ULTRACLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
UltraClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://ultraclaw.os/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/ultraclaw)
...
Environment variables:
  ULTRACLAW_INSTALL_METHOD=git|npm
  ULTRACLAW_VERSION=latest|next|<semver>
  ULTRACLAW_BETA=0|1
  ULTRACLAW_GIT_DIR=...
  ULTRACLAW_GIT_UPDATE=0|1
  ULTRACLAW_NO_PROMPT=1
  ULTRACLAW_DRY_RUN=1
  ULTRACLAW_NO_ONBOARD=1
  ULTRACLAW_VERBOSE=1
  ULTRACLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://ultraclaw.os/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://ultraclaw.os/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://ultraclaw.os/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected UltraClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a UltraClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_ultraclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"ultraclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://ultraclaw.os/install.sh | bash"
}
...
ensure_ultraclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/ultraclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/ultraclaw" ]]; then
        ln -sf "$npm_root/ultraclaw/dist/entry.js" "${npm_bin}/ultraclaw"
        ui_info "Created ultraclaw bin link at ${npm_bin}/ultraclaw"
    fi
    return 0
}

# Check for existing UltraClaw installation
check_existing_ultraclaw() {
    if [[ -n "$(type -P ultraclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing UltraClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make ultraclaw show as \"command not found\" in new terminals."
...
warn_ultraclaw_not_found() {
    ui_warn "Installed, but ultraclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t ultraclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named ultraclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_ultraclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P ultraclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/ultraclaw" ]]; then
        echo "${npm_bin}/ultraclaw"
        return 0
    fi
...
install_ultraclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/ultraclaw/ultraclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing UltraClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing UltraClaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning UltraClaw" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building UltraClaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/ultraclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/ultraclaw"
    ui_success "UltraClaw wrapper installed to \$HOME/.local/bin/ultraclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view ultraclaw dist-tags.beta 2>/dev/null || true)"
...
install_ultraclaw() {
    local package_name="ultraclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            ULTRACLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="ultraclaw"
        else
            ULTRACLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${ULTRACLAW_VERSION}" ]]; then
        ULTRACLAW_VERSION="latest"
    fi

    local resolved_version=""
    resolved_version="$(npm view "${package_name}@${ULTRACLAW_VERSION}" version 2>/dev/null || true)"
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing UltraClaw v${resolved_version}"
    else
        ui_info "Installing UltraClaw (${ULTRACLAW_VERSION})"
    fi
    local install_spec=""
    if [[ "${ULTRACLAW_VERSION}" == "latest" ]]; then
        install_spec="${package_name}@latest"
    else
        install_spec="${package_name}@${ULTRACLAW_VERSION}"
    fi

    if ! install_ultraclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_ultraclaw_paths
        install_ultraclaw_npm "${install_spec}"
    fi

    if [[ "${ULTRACLAW_VERSION}" == "latest" && "${package_name}" == "ultraclaw" ]]; then
        if ! resolve_ultraclaw_bin &> /dev/null; then
            ui_warn "npm install ultraclaw@latest failed; retrying ultraclaw@next"
            cleanup_npm_ultraclaw_paths
            install_ultraclaw_npm "ultraclaw@next"
        fi
    fi

    ensure_ultraclaw_bin_link || true

    ui_success "UltraClaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${ULTRACLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_ultraclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (ultraclaw not on PATH yet)"
        warn_ultraclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${ULTRACLAW_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.ultraclaw/workspace-${profile}"
    else
        echo "${HOME}/.ultraclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${ULTRACLAW_CONFIG_PATH:-$HOME/.ultraclaw/ultraclaw.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run ultraclaw onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but ultraclaw not on PATH; skipping onboarding"
        warn_ultraclaw_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run ultraclaw onboard to retry"
        return
    }
}
...
resolve_ultraclaw_version() {
    local version=""
    local raw_version_output=""
    local claw="${ULTRACLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v ultraclaw &> /dev/null; then
        claw="$(command -v ultraclaw)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_ultraclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/ultraclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/ultraclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_ultraclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found UltraClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set ULTRACLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_ultraclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing UltraClaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g ultraclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g ultraclaw 2>/dev/null || true
        ...
        install_ultraclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/ultraclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/ultraclaw"
    ...
    ULTRACLAW_BIN="$(resolve_ultraclaw_bin || true)"
    ...
    installed_version=$(resolve_ultraclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 UltraClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 UltraClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/ultraclaw"
        ui_kv "Update command" "ultraclaw update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://ultraclaw.os/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running ultraclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                ULTRACLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                ULTRACLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                ULTRACLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run ultraclaw doctor and ultraclaw plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run ultraclaw onboard later"
        else
            local config_path="${ULTRACLAW_CONFIG_PATH:-$HOME/.ultraclaw/ultraclaw.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (ultraclaw not on PATH yet)"
                    warn_ultraclaw_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run ultraclaw onboard to finish setup"
            return 0
        fi
    fi

    if command -v ultraclaw &> /dev/null; then
        local claw="${ULTRACLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_ultraclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (ultraclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if ULTRACLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: ultraclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${ULTRACLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
