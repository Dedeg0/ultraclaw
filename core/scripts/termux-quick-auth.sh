...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/ultraclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "UltraClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/ultraclaw/scripts/mobile-reauth.sh'" --id ultraclaw-auth
        ;;
    *)
