 ...
 * `ultraclaw update`. Use it to verify the completion installation flow
...
\`ultraclaw update\`, \`ultraclaw doctor\`, and \`ultraclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# UltraClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`ultraclaw completion --install\` later to enable.`));
    return;
  }
