...
type DriverMode = "token" | "webhook" | "ultraclaw";
...
function resolveStateDir(): string {
  const override = process.env.ULTRACLAW_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.ULTRACLAW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".ultraclaw");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver ultraclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) UltraClaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|ultraclaw> Driver transport mode (default: token)\n" +
    ...
    "  --ultraclaw-bin <path>        UltraClaw CLI binary for driver=ultraclaw (default: ultraclaw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  ULTRACLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  ULTRACLAW_DISCORD_SMOKE_DRIVER\n" +
    "  ULTRACLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  ULTRACLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  ULTRACLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  ULTRACLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  ULTRACLAW_DISCORD_SMOKE_AGENT\n" +
    "  ULTRACLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  ULTRACLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  ULTRACLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  ULTRACLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  ULTRACLAW_DISCORD_SMOKE_ULTRACLAW_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.ULTRACLAW_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.ULTRACLAW_DISCORD_SMOKE_DRIVER ||
  ...
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "ultraclaw"
        ? "ultraclaw"
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.ULTRACLAW_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.ULTRACLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.ULTRACLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.ULTRACLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.ULTRACLAW_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.ULTRACLAW_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.ULTRACLAW_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.ULTRACLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.ULTRACLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.ULTRACLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const ultraclawBin =
    resolveArg("--ultraclaw-bin") || process.env.ULTRACLAW_DISCORD_SMOKE_ULTRACLAW_BIN || "ultraclaw";
...
async function ultraclawCliJson<T>(params: { ultraclawBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.ultraclawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`ultraclaw ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await ultraclawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    ultraclawBin: params.ultraclawBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "ultraclaw") {
    return await readMessagesWithOpenclaw({
      ultraclawBin: params.args.ultraclawBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `ultraclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await ultraclawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        ultraclawBin: args.ultraclawBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("ultraclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from UltraClaw.`,
        diagnostics: {
          bindingCandidates: [
            {
