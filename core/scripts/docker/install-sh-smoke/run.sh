...
INSTALL_URL="${ULTRACLAW_INSTALL_URL:-https://ultraclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${ULTRACLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ULTRACLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="ultraclaw"
PACKAGE_NAME="${ULTRACLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ULTRACLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ULTRACLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ULTRACLAW_INSTALL_LATEST_OUT:-}"
fi
