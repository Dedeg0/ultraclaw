...
IMAGE_NAME="ultraclaw-gateway-network-e2e"

PORT="18790"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="ultraclaw-net-e2e-$$"
GW_NAME="ultraclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "ULTRACLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "ULTRACLAW_SKIP_CHANNELS=1" \
  -e "ULTRACLAW_SKIP_GMAIL_WATCHER=1" \
  -e "ULTRACLAW_SKIP_CRON=1" \
  -e "ULTRACLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
