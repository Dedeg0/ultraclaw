# UltraClaw ACP Bridge

This document describes how the UltraClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`ultraclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
UltraClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing UltraClaw Gateway
...
- ACP client spawns `ultraclaw acp` and speaks ACP messages over stdio.
...
`ultraclaw acp` resolves the Gateway URL and auth from CLI flags or config:
