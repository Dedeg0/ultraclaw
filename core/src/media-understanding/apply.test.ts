...
const TEMP_MEDIA_PREFIX = "ultraclaw-media-";
...
function createGroqAudioConfig(): UltraClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): UltraClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): UltraClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: UltraClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredUltraClawTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        ULTRACLAW_AGENT_DIR: isolatedAgentDir,
