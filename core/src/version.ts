...
declare const __ULTRACLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "ultraclaw";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["ULTRACLAW_VERSION"],
      runtimeVersion,
      env["ULTRACLAW_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current UltraClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __ULTRACLAW_VERSION__ === "string" ? __ULTRACLAW_VERSION__ : undefined,
  bundledVersion: process.env.ULTRACLAW_BUNDLED_VERSION,
});
