...
// Prefix for UltraClaw-generated action IDs to scope our handler
const ULTRACLAW_ACTION_PREFIX = "ultraclaw:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${ULTRACLAW_ACTION_PREFIX}`);

  // Handle UltraClaw modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
