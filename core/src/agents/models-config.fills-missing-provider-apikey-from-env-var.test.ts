...
async function runCustomProviderMergeTest(params: {
  ...
  await ensureUltraClawModelsJson({
    models: {
      mode: "merge",
      providers: {
...
function createOpenAiConfigWithResolvedApiKey(mergeMode = false): UltraClawConfig {
  return {
    models: {
...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureUltraClawModelsJson(validated.config);
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureUltraClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
