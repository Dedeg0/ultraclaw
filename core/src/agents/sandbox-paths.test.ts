...
import { resolvePreferredUltraClawTmpDir } from "../infra/tmp-ultraclaw-dir.js";
...
async function withOutsideHardlinkInUltraClawTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside ultraclaw tmp root",
      media: path.join(os.tmpdir(), "outside-ultraclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked UltraClaw tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked UltraClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInUltraClawTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked UltraClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInUltraClawTmp(
