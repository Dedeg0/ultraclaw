...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("ultraclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("ultraclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("ultraclaw-sbx-");
