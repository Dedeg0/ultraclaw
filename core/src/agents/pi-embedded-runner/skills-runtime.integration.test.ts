...
const originalBundledDir = process.env.ULTRACLAW_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("ultraclaw-bundled-");
  const workspaceDir = await createTempDir("ultraclaw-workspace-");
...
afterEach(async () => {
  process.env.ULTRACLAW_BUNDLED_PLUGINS_DIR = originalBundledDir;
...
describe("resolveEmbeddedRunSkillEntries (integration)", () => {
  it("loads bundled diffs skill when explicitly enabled in config", async () => {
    ...
    process.env.ULTRACLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
    clearPluginManifestRegistryCache();
