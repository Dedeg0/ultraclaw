...
import { markUltraClawExecEnv } from "../../infra/ultraclaw-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "ultraclaw.sandbox=1");
  args.push("--label", `ultraclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `ultraclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `ultraclaw.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markUltraClawExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "ultraclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`ultraclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`ultraclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("ultraclaw sandbox recreate --all");
}
