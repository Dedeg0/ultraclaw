...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    ultraclaw: process.env.ULTRACLAW_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.ultraclaw === undefined) {
      delete process.env.ULTRACLAW_GATEWAY_TOKEN;
    } else {
      process.env.ULTRACLAW_GATEWAY_TOKEN = envSnapshot.ultraclaw;
    }
  ...
  it("uses ULTRACLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.ULTRACLAW_GATEWAY_TOKEN = "local-env-token";
