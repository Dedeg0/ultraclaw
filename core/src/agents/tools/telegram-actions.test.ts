...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): UltraClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as UltraClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): UltraClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as UltraClawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: UltraClawConfig, accountId = "media") {
    await handleTelegramAction(
