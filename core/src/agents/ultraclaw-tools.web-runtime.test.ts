...
function findTool(name: string, config: UltraClawConfig) {
  const allTools = createUltraClawTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: UltraClawConfig; env?: NodeJS.ProcessEnv }) {
...
describe("ultraclaw tools runtime web metadata wiring", () => {
