...
import { planUltraClawModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: UltraClawConfig): {
  config: UltraClawConfig;
  sourceConfigForSecrets: UltraClawConfig;
} {
...
export async function ensureUltraClawModelsJson(
  config?: UltraClawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveUltraClawAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planUltraClawModelsJson({
      cfg,
