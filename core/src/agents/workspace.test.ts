...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ULTRACLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ULTRACLAW_HOME: "/srv/ultraclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/ultraclaw-home"), ".ultraclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".ultraclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("ultraclaw-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-workspace-hardlink-"));
    try {
