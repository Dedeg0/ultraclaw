...
const GOOGLE_LIVE_SINGLE_KEY = "ULTRACLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ULTRACLAW_LIVE_ANTHROPIC_KEY",
    listVar: "ULTRACLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ULTRACLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ULTRACLAW_LIVE_${base}_KEY`;
