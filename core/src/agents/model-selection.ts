...
export function isCliProvider(provider: string, cfg?: UltraClawConfig): boolean {
  const normalized = normalizeProviderId(provider);
...
function resolveAllowedFallbacks(params: { cfg: UltraClawConfig; agentId?: string }): string[] {
  if (params.agentId) {
