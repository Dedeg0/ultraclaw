    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "ultraclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "ultraclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "ultraclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("ultraclaw-");
  const acpxRoot = await tempDirs.make("ultraclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("ultraclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("ultraclaw-");
  const pluginRoot = await tempDirs.make("ultraclaw-plugin-");
  const outsideDir = await tempDirs.make("ultraclaw-outside-");
