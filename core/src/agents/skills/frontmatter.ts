...
import {
  applyUltraClawManifestInstallCommonFields,
  getFrontmatterString,
  normalizeStringList,
  parseUltraClawManifestInstallBase,
  parseFrontmatterBool,
  resolveUltraClawManifestBlock,
  resolveUltraClawManifestInstall,
  resolveUltraClawManifestOs,
  resolveUltraClawManifestRequires,
} from "../../shared/frontmatter.js";
import type {
  UltraClawSkillMetadata,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseUltraClawManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyUltraClawManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveUltraClawMetadata(
  frontmatter: ParsedSkillFrontmatter,
): UltraClawSkillMetadata | undefined {
  const metadataObj = resolveUltraClawManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveUltraClawManifestRequires(metadataObj);
  const install = resolveUltraClawManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveUltraClawManifestOs(metadataObj);
  return {
