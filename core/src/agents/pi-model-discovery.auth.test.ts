...
async function createAgentDir(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-pi-auth-storage-"));
}
...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    await withAgentDir(async (agentDir) => {
      const previous = process.env.ULTRACLAW_AUTH_STORE_READONLY;
      ...
      try {
        ...
        if (previous === undefined) {
          delete process.env.ULTRACLAW_AUTH_STORE_READONLY;
        } else {
          process.env.ULTRACLAW_AUTH_STORE_READONLY = previous;
        }
      }
    });
  });
