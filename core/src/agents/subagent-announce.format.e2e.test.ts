...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/ultraclaw/ultraclaw/issues/31298
    previousFastTestEnv = process.env.ULTRACLAW_TEST_FAST;
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.ULTRACLAW_TEST_FAST;
      return;
    }
    process.env.ULTRACLAW_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // ULTRACLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("UltraClaw runtime context (internal):");
