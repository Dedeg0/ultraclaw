...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["ULTRACLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-subagent-"));
  ...
  it("uses isolated temp state when ULTRACLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "ultraclaw-test-state"));
  });
});
