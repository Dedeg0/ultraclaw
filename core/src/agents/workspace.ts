...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.ULTRACLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".ultraclaw", `workspace-${profile}`);
  }
  return path.join(home, ".ultraclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".ultraclaw";
