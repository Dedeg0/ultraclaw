 ...
 * A common mistake — caused by the similarity with the `ultraclaw.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (ultraclaw.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.ULTRACLAW_AUTH_STORE_READONLY === "1";
