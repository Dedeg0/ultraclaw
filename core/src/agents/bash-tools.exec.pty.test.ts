...
test("exec sets ULTRACLAW_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-ultraclaw-shell", {
    command: "node -e \"process.stdout.write(process.env.ULTRACLAW_SHELL || '')\"",
    pty: true,
  });
