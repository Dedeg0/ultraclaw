...
function resolveAttachmentLimits(config: UltraClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".ultraclaw", "attachments");
  const relDir = path.posix.join(".ultraclaw", "attachments", attachmentId);
