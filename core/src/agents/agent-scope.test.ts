...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: UltraClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/ultraclaw" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: UltraClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: UltraClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: UltraClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: UltraClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: UltraClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: UltraClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: UltraClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: UltraClawConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/ultraclaw-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/ultraclaw");
  });

  it("uses ULTRACLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "ultraclaw-home");
    vi.stubEnv("ULTRACLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as UltraClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".ultraclaw", "workspace"));
  });

  it("uses ULTRACLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "ultraclaw-home");
    vi.stubEnv("ULTRACLAW_HOME", home);
    // Clear state dir so it falls back to ULTRACLAW_HOME
    vi.stubEnv("ULTRACLAW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as UltraClawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".ultraclaw", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/ultraclaw-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/ultraclaw-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "ultraclaw-agent-scope-"));
