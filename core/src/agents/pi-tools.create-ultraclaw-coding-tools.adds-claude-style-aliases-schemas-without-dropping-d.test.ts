...
describe("createUltraClawCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-read-"));
    try {
      ...
      const contents = "Hello from ultraclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createUltraClawCodingTools({ sandbox });
