...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("ultraclaw-ws-", async (workspaceDir) => {
      await withTempDir("ultraclaw-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createUltraClawCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("ultraclaw-ws-", async (workspaceDir) => {
      await withTempDir("ultraclaw-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("ultraclaw-ws-", async (workspaceDir) => {
      const cfg: UltraClawConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createUltraClawCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-ultraclaw.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("ultraclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("ultraclaw-workspace-", async (workspaceDir) => {
        ...
        const tools = createUltraClawCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
