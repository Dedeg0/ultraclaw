...
type CoreToolDefinition = {
  ...
  includeInUltraClawGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const ultraclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInUltraClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:ultraclaw": ultraclawTools,
