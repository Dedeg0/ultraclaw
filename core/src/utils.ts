...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".ultraclaw");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ULTRACLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ULTRACLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via ULTRACLAW_STATE_DIR.
