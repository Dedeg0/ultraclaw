...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ULTRACLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-ultraclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-ultraclaw#general");
  });
  ...
  it("derives session transcripts dir from ULTRACLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ULTRACLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when ULTRACLAW_STATE_DIR differs from stored paths", () => {
