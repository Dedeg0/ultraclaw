...
function collectConfigEnvVarsByTarget(cfg?: UltraClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: UltraClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: UltraClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: UltraClawConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.ULTRACLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
