...
import type { UltraClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareUltraClawVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  ultraclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  ultraclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: UltraClawConfig,
  pathSegments: string[],
): { changed: boolean; next: UltraClawConfig } {
...
function coerceConfig(value: unknown): UltraClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as UltraClawConfig;
}
...
function stampConfigVersion(cfg: UltraClawConfig): UltraClawConfig {
...
function warnIfConfigFromFuture(cfg: UltraClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareUltraClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer UltraClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as UltraClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): UltraClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as UltraClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: UltraClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as UltraClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.ULTRACLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.ULTRACLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.ULTRACLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.ULTRACLAW_WATCH_SESSION === "string" &&
        deps.env.ULTRACLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.ULTRACLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.ULTRACLAW_WATCH_COMMAND === "string" &&
        deps.env.ULTRACLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.ULTRACLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `ULTRACLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: UltraClawConfig | null = null;
let runtimeConfigSourceSnapshot: UltraClawConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ULTRACLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ULTRACLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): UltraClawConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): UltraClawConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: UltraClawConfig;
  candidate: UltraClawConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: UltraClawConfig): UltraClawConfig {
...
export function loadConfig(): UltraClawConfig {
...
export async function readBestEffortConfig(): Promise<UltraClawConfig> {
...
export async function writeConfigFile(
  ...
  // edits to ultraclaw.json remain visible (no stale snapshot).
}
