...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-config-"));
  try {
    await run(home);
  } finally {
...
async function writeConfig(
  ...
  dirname: ".ultraclaw",
  port: number,
  filename: string = "ultraclaw.json",
) {
...
describe("config io paths", () => {
  it("uses ~/.ultraclaw/ultraclaw.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".ultraclaw", 19001);
  ...
  it("defaults to ~/.ultraclaw/ultraclaw.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".ultraclaw", "ultraclaw.json"));
    });
  });

  it("uses ULTRACLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { ULTRACLAW_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".ultraclaw", "ultraclaw.json"));
    });
  });

  it("honors explicit ULTRACLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".ultraclaw", 20002, "custom.json");
      const io = createIoForHome(home, { ULTRACLAW_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("honors legacy CLAWDBOT_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".ultraclaw", 20003, "legacy-custom.json");
  ...
  it("normalizes safe-bin config entries at config load time", async () => {
    await withTempHome(async (home) => {
      ...
      const configPath = path.join(configDir, "ultraclaw.json");
      await fs.writeFile(
        configPath,
        JSON.stringify(
          {
