...
import { buildWebSearchProviderConfig, withTempHome, writeUltraClawConfig } from "./test-helpers.js";
import { UltraClawSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = UltraClawSchema.safeParse({
      $schema: "https://ultraclaw.os/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://ultraclaw.os/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = UltraClawSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = UltraClawSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeUltraClawConfig(home, {
        agents: { list: [{ id: "pi" }] },
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeUltraClawConfig(home, {
        gateway: { bind: "${ULTRACLAW_BIND}" },
      });

      const prev = process.env.ULTRACLAW_BIND;
      process.env.ULTRACLAW_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.ULTRACLAW_BIND;
        } else {
          process.env.ULTRACLAW_BIND = prev;
        }
      }
    });
  });
