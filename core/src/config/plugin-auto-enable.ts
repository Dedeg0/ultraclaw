...
function isWhatsAppConfigured(cfg: UltraClawConfig): boolean {
...
function isGenericChannelConfigured(cfg: UltraClawConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: UltraClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: UltraClawConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: UltraClawConfig, env: NodeJS.ProcessEnv): string[] {
...
function isPluginExplicitlyDisabled(cfg: UltraClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: UltraClawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: UltraClawConfig, pluginId: string): UltraClawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
