...
import type { ConfigFileSnapshot } from "./types.ultraclaw.js";
import { UltraClawSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(UltraClawSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.ultraclaw/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/ultraclaw/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/ultraclaw/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/ultraclaw/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/ultraclaw/nickserv-password.txt");
...
describe("restoreRedactedValues", () => {
  ...
  it("preserves non-sensitive fields unchanged", () => {
    ...
    const original = {
      ui: { seamColor: "#0088cc" },
      gateway: { port: 18790, auth: { token: "real-secret" } },
    };
    const result = restoreRedactedValues(incoming, original) as typeof incoming;
  ...
  it("round-trips config through redact → restore", () => {
    const originalConfig = {
      gateway: { auth: { token: "gateway-auth-secret-token-value" }, port: 18790 },
      channels: {
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = UltraClawSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "UltraClawConfig";
    const hints = mainSchemaHints;
