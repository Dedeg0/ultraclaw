...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "ultraclaw.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      ULTRACLAW_STATE_DIR: path.join(suiteHome, ".ultraclaw"),
      CLAWDBOT_STATE_DIR: undefined,
      ULTRACLAW_PLUGIN_MANIFEST_CACHE_MS: "10000",
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-config-plugin-validation-"));
