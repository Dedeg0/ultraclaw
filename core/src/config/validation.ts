...
import type { UltraClawConfig, ConfigValidationIssue } from "./types.js";
import { UltraClawSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: UltraClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: UltraClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: UltraClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = UltraClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as UltraClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as UltraClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as UltraClawConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as UltraClawConfig,
  };
}
