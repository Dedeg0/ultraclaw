...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("ultraclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "ultraclaw-sandbox-browser-global" },
      agentBrowser: { network: "ultraclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("ultraclaw-sandbox-browser-agent");
  });
