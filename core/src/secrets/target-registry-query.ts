...
const ULTRACLAW_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of ULTRACLAW_COMPILED_SECRET_TARGETS) {
...
const ULTRACLAW_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: ULTRACLAW_COMPILED_SECRET_TARGETS,
    entriesById: ULTRACLAW_TARGETS_BY_ID,
  });
