...
export function buildConfigureCandidates(config: UltraClawConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredUltraClawConfig?: UltraClawConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredUltraClawConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const ultraclawCandidates = discoverConfigSecretTargets(params.config)
    ...
    .map((entry) => {
      ...
      return {
        ...
        label: entry.path,
        configFile: "ultraclaw.json" as const,
  ...
  return [...ultraclawCandidates, ...authCandidates].toSorted((a, b) =>
...
export function collectConfigureProviderChanges(params: {
  original: UltraClawConfig;
  next: UltraClawConfig;
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "ultraclaw secrets configure",
