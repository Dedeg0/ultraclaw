...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop ultraclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@ultraclaw_bot", { botUsername: "ultraclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@ultraclaw_bot", { botUsername: "ultraclaw_bot" })).toBe(true);
