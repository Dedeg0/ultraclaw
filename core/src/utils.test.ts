...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("ultraclaw-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("ultraclaw-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("ultraclaw-lid-a-", async (first) => {
      await withTempDir("ultraclaw-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.ultraclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "ultraclaw-config-dir-"));
    try {
  ...
  it("expands ULTRACLAW_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/ultraclaw-home",
      ULTRACLAW_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/ultraclaw-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers ULTRACLAW_HOME over HOME", () => {
    vi.stubEnv("ULTRACLAW_HOME", "/srv/ultraclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/ultraclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $ULTRACLAW_HOME prefix when ULTRACLAW_HOME is set", () => {
    vi.stubEnv("ULTRACLAW_HOME", "/srv/ultraclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/ultraclaw-home")}/.ultraclaw/ultraclaw.json`)).toBe(
      "$ULTRACLAW_HOME/.ultraclaw/ultraclaw.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $ULTRACLAW_HOME replacement when ULTRACLAW_HOME is set", () => {
    vi.stubEnv("ULTRACLAW_HOME", "/srv/ultraclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/ultraclaw-home")}/.ultraclaw/ultraclaw.json`),
    ).toBe("config: $ULTRACLAW_HOME/.ultraclaw/ultraclaw.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/ultraclaw")).toBe(path.resolve(os.homedir(), "ultraclaw"));
  });
  ...
  it("prefers ULTRACLAW_HOME for tilde expansion", () => {
    vi.stubEnv("ULTRACLAW_HOME", "/srv/ultraclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/ultraclaw")).toBe(path.resolve("/srv/ultraclaw-home", "ultraclaw"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/ultraclaw", env)).toBe(path.resolve("/srv/ultraclaw-home", "ultraclaw"));
  });
