...
async function withWindowsEnv(
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-win-startup-"));
  const env = {
    USERPROFILE: tmpDir,
...
async function writeGatewayScript(env: Record<string, string>, port = 18790) {
  ...
  await fs.writeFile(
    ...
    [
      "@echo off",
      `set "ULTRACLAW_GATEWAY_PORT=${port}"`,
      `"C:\\Program Files\\nodejs\\node.exe" "C:\\Users\\steipete\\AppData\\Roaming\\npm\\node_modules\\ultraclaw\\dist\\index.js" gateway --port ${port}`,
      "",
    ].join("\r\n"),
    "utf8",
  );
...
describe("Windows startup fallback", () => {
  it("falls back to a Startup-folder launcher when schtasks create is denied", async () => {
    await withWindowsEnv("ultraclaw-win-startup-", async ({ env }) => {
      ...
      const result = await installScheduledTask({
        ...
        environment: { ULTRACLAW_GATEWAY_PORT: "18790" },
      });
  ...
  it("kills the Startup fallback runtime even when the CLI env omits the gateway port", async () => {
    await withWindowsEnv(async ({ env }) => {
      ...
      delete envWithoutPort.ULTRACLAW_GATEWAY_PORT;
