...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when ULTRACLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".ultraclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when ULTRACLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", ULTRACLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".ultraclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers ULTRACLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        ULTRACLAW_PROFILE: "rescue",
        ULTRACLAW_STATE_DIR: "C:\\State\\ultraclaw",
      },
      expected: path.join("C:\\State\\ultraclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", ULTRACLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".ultraclaw", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem UltraClaw Gateway",
          "cd /d C:\\Projects\\ultraclaw",
          "set NODE_ENV=production",
          "set ULTRACLAW_PORT=18790",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\ultraclaw",
          environment: {
            NODE_ENV: "production",
            ULTRACLAW_PORT: "18790",
          },
          sourcePath: resolveTaskScriptPath(env),
        });
      },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\ultraclaw\\dist\\index.js gateway --port 18790',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\ultraclaw\\dist\\index.js",
            "gateway",
            "--port",
            "18790",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\UltraClaw Share\\node.exe" "\\\\fileserver\\UltraClaw Share\\dist\\index.js" gateway --port 18790',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\UltraClaw Share\\node.exe",
            "\\\\fileserver\\UltraClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18790",
          ],
  ...
  it("reads script from ULTRACLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ ULTRACLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
