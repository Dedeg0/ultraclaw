...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `ultraclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      ULTRACLAW_LAUNCHD_LABEL: `ai.ultraclaw.launchd-int-${testId}`,
      ULTRACLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
