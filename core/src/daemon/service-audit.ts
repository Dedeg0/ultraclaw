...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds ULTRACLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `ultraclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service ULTRACLAW_GATEWAY_TOKEN does not match gateway.auth.token in ultraclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.ULTRACLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.ULTRACLAW_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `ultraclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
