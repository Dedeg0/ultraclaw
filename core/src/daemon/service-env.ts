...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.ULTRACLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ULTRACLAW_PROFILE: profile,
    ULTRACLAW_GATEWAY_PORT: String(port),
    ULTRACLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    ULTRACLAW_SYSTEMD_UNIT: systemdUnit,
    ULTRACLAW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    ULTRACLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    ULTRACLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.ULTRACLAW_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ULTRACLAW_GATEWAY_TOKEN: gatewayToken,
    ULTRACLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    ULTRACLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    ULTRACLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    ULTRACLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    ULTRACLAW_LOG_PREFIX: "node",
    ULTRACLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    ULTRACLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    ULTRACLAW_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    ULTRACLAW_STATE_DIR: sharedEnv.stateDir,
    ULTRACLAW_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.ULTRACLAW_STATE_DIR;
  const configPath = env.ULTRACLAW_CONFIG_PATH;
