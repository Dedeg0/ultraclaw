...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ULTRACLAW_GATEWAY_PORT).toBe("18790");
    expect(env.ULTRACLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.ULTRACLAW_SERVICE_MARKER).toBe("ultraclaw");
    expect(env.ULTRACLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ULTRACLAW_SERVICE_VERSION).toBe("string");
    expect(env.ULTRACLAW_SYSTEMD_UNIT).toBe("ultraclaw-gateway.service");
    expect(env.ULTRACLAW_WINDOWS_TASK_NAME).toBe("UltraClaw Gateway");
    if (process.platform === "darwin") {
      expect(env.ULTRACLAW_LAUNCHD_LABEL).toBe("ai.ultraclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ULTRACLAW_PROFILE: "work" },
      port: 18790,
    });
    expect(env.ULTRACLAW_SYSTEMD_UNIT).toBe("ultraclaw-gateway-work.service");
    expect(env.ULTRACLAW_WINDOWS_TASK_NAME).toBe("UltraClaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.ULTRACLAW_LAUNCHD_LABEL).toBe("ai.ultraclaw.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through ULTRACLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", ULTRACLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.ULTRACLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to ULTRACLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.ULTRACLAW_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers ULTRACLAW_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        ULTRACLAW_GATEWAY_TOKEN: "ultraclaw-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.ULTRACLAW_GATEWAY_TOKEN).toBe("ultraclaw-token");
  });

  it("omits ULTRACLAW_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".ultraclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", ULTRACLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".ultraclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", ULTRACLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".ultraclaw"));
  });

  it("uses ULTRACLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", ULTRACLAW_STATE_DIR: "/var/lib/ultraclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/ultraclaw"));
  });

  it("expands ~ in ULTRACLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", ULTRACLAW_STATE_DIR: "~/ultraclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/ultraclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { ULTRACLAW_STATE_DIR: "C:\\State\\ultraclaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\ultraclaw");
  });
});
