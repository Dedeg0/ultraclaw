...
import { ensureUltraClawExecMarkerOnProcess } from "./infra/ultraclaw-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "ultraclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "ultraclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.ULTRACLAW_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.ULTRACLAW_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.ULTRACLAW_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.ULTRACLAW_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[ultraclaw] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `UltraClaw ${VERSION} (${commit})` : `UltraClaw ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[ultraclaw] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[ultraclaw] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[ultraclaw] ${parsed.error}`);
      process.exit(2);
    }
