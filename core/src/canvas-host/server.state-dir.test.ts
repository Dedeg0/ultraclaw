...
describe("canvas host state dir defaults", () => {
  it("uses ULTRACLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("ultraclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("UltraClaw Canvas");
      } finally {
        await handler.close();
      }
    });
