...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "ultraclaw-install-sh-"));
  const cliPath = path.join(root, "ultraclaw");
...
ULTRACLAW_BIN="$FAKE_ULTRACLAW_BIN"
resolve_ultraclaw_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_ULTRACLAW_BIN: cliPath,
        ULTRACLAW_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("UltraClaw 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("UltraClaw dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("UltraClaw dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "ultraclaw-install-stdin-"));
