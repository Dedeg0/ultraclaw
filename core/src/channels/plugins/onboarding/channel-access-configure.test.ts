...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: UltraClawConfig, policy: ChannelAccessPolicy) => UltraClawConfig;
  resolveAllowlist: (params: { cfg: UltraClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: UltraClawConfig; resolved: TResolved }) => UltraClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: UltraClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: UltraClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: UltraClawConfig, policy: ChannelAccessPolicy): UltraClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: UltraClawConfig, policy: ChannelAccessPolicy): UltraClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: UltraClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: UltraClawConfig; resolved: string[] }) => {
      calls.push("apply");
