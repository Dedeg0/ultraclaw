...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: UltraClawConfig) => Array<string | number> | undefined;
}): UltraClawConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: UltraClawConfig) => UltraClawConfig | Promise<UltraClawConfig>;
  applySet?: (
    cfg: UltraClawConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => UltraClawConfig | Promise<UltraClawConfig>;
}): Promise<{
  cfg: UltraClawConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in UltraClaw config",
  ...
  const resolved = await promptSecretRefForOnboarding({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "ULTRACLAW_SECRET",
      envVarFormatError:
        'Use an env var name like "ULTRACLAW_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
