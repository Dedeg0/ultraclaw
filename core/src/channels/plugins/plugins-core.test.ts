...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@ultraclaw/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "ultraclaw-catalog-"));
        ...
        entries: [
          {
            ...
            ultraclaw: {
              ...
              install: {
                npmSpec: "@ultraclaw/demo-channel",
              },
            },
          },
        ],
  ...
  it("uses the provided env for external catalog path resolution", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "ultraclaw-catalog-home-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@ultraclaw/env-demo-channel",
            ultraclaw: {
              channel: {
                id: "env-demo-channel",
              ...
              install: {
                npmSpec: "@ultraclaw/env-demo-channel",
              },
            },
          },
        ],
    ...
    const ids = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        ULTRACLAW_PLUGIN_CATALOG_PATHS: "~/catalog.json",
        HOME: home,
      },
    }).map((entry) => entry.id);
  ...
  it("uses the provided env for default catalog paths", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "ultraclaw-catalog-state-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@ultraclaw/default-env-demo",
            ultraclaw: {
              channel: {
                id: "default-env-demo",
              ...
              install: {
                npmSpec: "@ultraclaw/default-env-demo",
              },
            },
          },
        ],
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: UltraClawConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
