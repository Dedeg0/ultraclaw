...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update UltraClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["ultraclaw update", "Update a source checkout (git)"],
        ["ultraclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["ultraclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["ultraclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["ultraclaw update --dry-run", "Preview actions without changing anything"],
        ["ultraclaw update --no-restart", "Update without restarting the service"],
        ["ultraclaw update --json", "Output result as JSON"],
        ["ultraclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["ultraclaw update wizard", "Interactive update wizard"],
        ["ultraclaw --update", "Shorthand for ultraclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run ultraclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.ultraclaw.os/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["ultraclaw update status", "Show channel + version status."],
          ["ultraclaw update status --json", "JSON output."],
          ["ultraclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.ultraclaw.os/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
