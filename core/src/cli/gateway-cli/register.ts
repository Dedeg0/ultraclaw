      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["ultraclaw gateway run", "Run the gateway in the foreground."],
            ["ultraclaw gateway status", "Show service status and probe reachability."],
            ["ultraclaw gateway discover", "Find local and wide-area gateway beacons."],
            ["ultraclaw gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.ultraclaw.os/cli/gateway")}\n`,
      ),
  );
