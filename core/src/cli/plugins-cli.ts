...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: UltraClawConfig; warnings: string[] } {
...
async function runPluginInstallCommand(params: {
  ...
  if (fs.existsSync(resolved)) {
    if (opts.link) {
      ...
      let next: UltraClawConfig = enablePluginInConfig(
        {
          ...cfg,
          plugins: {
            ...cfg.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage UltraClaw plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.ultraclaw.os/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: UltraClawConfig = enableResult.config;
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.ultraclaw.os/plugin");
      lines.push("");
