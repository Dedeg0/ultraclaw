...
import type { UltraClawConfig, ConfigFileSnapshot } from "../config/types.ultraclaw.js";
...
vi.mock("../infra/ultraclaw-root.js", () => ({
  resolveUltraClawPackageRoot: vi.fn(),
}));
...
const { resolveUltraClawPackageRoot } = await import("../infra/ultraclaw-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/ultraclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as UltraClawConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/ultraclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveUltraClawPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("ultraclaw-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveUltraClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("ultraclaw");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/ultraclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18790,
      status: "busy",
      listeners: [{ pid: 4242, command: "ultraclaw-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("UltraClaw update status");
  });
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as UltraClawConfig,
        });
      },
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ["npm", "i", "-g", "ultraclaw@latest", "--no-fund", "--no-audit", "--loglevel=error"],
      expect.any(Object),
    );
  });
  ...
  it("honors --tag override", async () => {
    ...
    expect(runGatewayUpdate).not.toHaveBeenCalled();
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ["npm", "i", "-g", "ultraclaw@next", "--no-fund", "--no-audit", "--loglevel=error"],
      expect.any(Object),
    );
  });
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("ultraclaw-update");
    const localAppData = createCaseDir("ultraclaw-localappdata");
  ...
  it("uses ULTRACLAW_UPDATE_PACKAGE_SPEC for package updates", async () => {
    const tempDir = createCaseDir("ultraclaw-update");
    mockPackageInstallStatus(tempDir);
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      [
        ...
        "i",
        "-g",
        "http://10.211.55.2:8138/ultraclaw-next.tgz",
        "--no-fund",
        "--no-audit",
        "--loglevel=error",
      ],
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("ultraclaw-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ ULTRACLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.ULTRACLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("ultraclaw-update-wizard");
    await withEnvAsync({ ULTRACLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
