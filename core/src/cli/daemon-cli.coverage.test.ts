...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      ULTRACLAW_GATEWAY_PORT: String(params.port),
      ...(params.token ? { ULTRACLAW_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ULTRACLAW_STATE_DIR = "/tmp/ultraclaw-cli-state";
    process.env.ULTRACLAW_CONFIG_PATH = "/tmp/ultraclaw-cli-state/ultraclaw.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        ULTRACLAW_STATE_DIR: "/tmp/ultraclaw-daemon-state",
        ULTRACLAW_CONFIG_PATH: "/tmp/ultraclaw-daemon-state/ultraclaw.json",
        ULTRACLAW_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.ultraclaw.gateway.plist",
    });
