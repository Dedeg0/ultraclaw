...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "ultraclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "ultraclaw-bundled-skills-test-"));
    process.env.ULTRACLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
