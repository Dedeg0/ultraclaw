    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["ultraclaw memory status", "Show index and provider status."],
          ["ultraclaw memory status --deep", "Probe embedding provider readiness."],
          ["ultraclaw memory index --force", "Force a full reindex."],
          ['ultraclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'ultraclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["ultraclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.ultraclaw.os/cli/memory")}\n`,
    );
