...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "ultraclaw";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "ultraclaw"}`,
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "ultraclaw" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option(
      "--driver <driver>",
      "Profile driver (ultraclaw|extension|existing-session). Default: ultraclaw",
    )
    .action(
