...
const resolveGatewayPort = vi.fn(() => 18790);
...
describe("runDaemonRestart health checks", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["ultraclaw", "gateway", "--port", "18790"],
      environment: {},
    });
    ...
    mockReadFileSync.mockImplementation((path: string) => {
      ...
      if ([4200, 4300].includes(pid)) {
        return ["ultraclaw", "gateway", "--port", "18790", ""].join("\0");
      }
    ...
    mockSpawnSync.mockReturnValue({
      error: null,
      status: 0,
      stdout: "ultraclaw gateway --port 18790",
      stderr: "",
    });
  });
  ...
  it("fails restart when gateway remains unhealthy", async () => {
    ...
    await expect(runDaemonRestart({ json: true })).rejects.toMatchObject({
      ...
      hints: ["ultraclaw gateway status --deep", "ultraclaw doctor"],
    });
  ...
  it("signals an unmanaged gateway process on stop", async () => {
    ...
    mockSpawnSync.mockReturnValue({
      ...
      status: 0,
      stdout:
        'CommandLine="C:\\\\Program Files\\\\UltraClaw\\\\ultraclaw.exe" gateway --port 18790\r\n',
      stderr: "",
    });
    ...
    expect(findGatewayPidsOnPortSync).toHaveBeenCalledWith(18790);
