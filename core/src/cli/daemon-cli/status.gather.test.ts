...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    ULTRACLAW_STATE_DIR: "/tmp/ultraclaw-daemon",
    ULTRACLAW_CONFIG_PATH: "/tmp/ultraclaw-daemon/ultraclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.ULTRACLAW_STATE_DIR ?? "/tmp/ultraclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.ULTRACLAW_CONFIG_PATH ?? `${stateDir}/ultraclaw.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/ultraclaw-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ULTRACLAW_STATE_DIR = "/tmp/ultraclaw-cli";
    process.env.ULTRACLAW_CONFIG_PATH = "/tmp/ultraclaw-cli/ultraclaw.json";
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.ULTRACLAW_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
