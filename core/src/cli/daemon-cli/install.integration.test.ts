...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("ultraclaw-daemon-install-int-");
    configPath = path.join(tempHome, "ultraclaw.json");
    process.env.HOME = tempHome;
    process.env.ULTRACLAW_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.ULTRACLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});
