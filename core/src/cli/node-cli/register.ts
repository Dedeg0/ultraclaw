    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "ultraclaw node run --host 127.0.0.1 --port 18790",
            "Run the node host in the foreground.",
          ],
          ["ultraclaw node status", "Check node host service status."],
          ["ultraclaw node install", "Install the node host service."],
          ["ultraclaw node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.ultraclaw.os/cli/node")}\n`,
    );
