...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.ultraclaw.os/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + UltraClaw hooks")
    ...
    .option("--hook-url <url>", "UltraClaw hook URL")
    .option("--hook-token <token>", "UltraClaw hook token")
