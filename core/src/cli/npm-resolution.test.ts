...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@ultraclaw/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@ultraclaw/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@ultraclaw/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@ultraclaw/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @ultraclaw/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@ultraclaw/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@ultraclaw/plugin-alpha@1.2.3",
        installPath: "/tmp/ultraclaw/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@ultraclaw/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @ultraclaw/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@ultraclaw/plugin-alpha@latest",
      true,
      "/tmp/ultraclaw/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@ultraclaw/plugin-alpha@latest",
      installPath: "/tmp/ultraclaw/extensions/alpha",
      version: "1.2.3",
