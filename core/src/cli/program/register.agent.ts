...
${formatHelpExamples([
  ['ultraclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['ultraclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'ultraclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'ultraclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['ultraclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'ultraclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.ultraclaw.os/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.ultraclaw.os/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['ultraclaw agents set-identity --agent main --name "UltraClaw" --emoji "🦞"', "Set name + emoji."],
  ["ultraclaw agents set-identity --agent main --avatar avatars/ultraclaw.png", "Set avatar path."],
  [
    "ultraclaw agents set-identity --workspace ~/.ultraclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "ultraclaw agents set-identity --identity-file ~/.ultraclaw/workspace/IDENTITY.md --agent main",
