    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["ultraclaw nodes status", "List known nodes with live status."],
          ["ultraclaw nodes pairing pending", "Show pending node pairing requests."],
          ['ultraclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["ultraclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.ultraclaw.os/cli/nodes")}\n`,
    );
