...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("skips integrity drift checks for unpinned npm specs during dry-run updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "opik-ultraclaw",
      targetDir: "/tmp/opik-ultraclaw",
      version: "0.2.6",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-ultraclaw": {
              source: "npm",
              spec: "@opik/opik-ultraclaw",
              integrity: "sha512-old",
              installPath: "/tmp/opik-ultraclaw",
            },
          },
        },
      },
      pluginIds: ["opik-ultraclaw"],
      dryRun: true,
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-ultraclaw": {
              source: "npm",
              spec: "@opik/opik-ultraclaw@0.2.5",
              integrity: "sha512-old",
              installPath: "/tmp/opik-ultraclaw",
            },
          },
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @ultraclaw/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @ultraclaw/missing.",
      },
    ]);
  });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/ultraclaw-home";
    resolveBundledPluginSourcesMock.mockReturnValue(
      new Map([
        [
          "feishu",
