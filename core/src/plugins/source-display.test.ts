...
describe("formatPluginSourceForTable", () => {
  it("shortens bundled plugin sources under the stock root", () => {
    ...
    const globalRoot = path.resolve(path.sep, "Users", "x", ".ultraclaw", "extensions");
    const workspaceRoot = path.resolve(path.sep, "Users", "x", "ws", ".ultraclaw", "extensions");
  ...
  it("resolves source roots from an explicit env override", () => {
    ...
    const homeDir = path.resolve(path.sep, "tmp", "ultraclaw-home");
    const roots = withEnv(
      {
        ULTRACLAW_BUNDLED_PLUGINS_DIR: path.join(ignoredHome, "ignored-bundled"),
        ULTRACLAW_STATE_DIR: path.join(ignoredHome, "ignored-state"),
        HOME: ignoredHome,
      },
      () =>
        resolvePluginSourceRoots({
          env: {
            ...process.env,
            HOME: homeDir,
            ULTRACLAW_BUNDLED_PLUGINS_DIR: "~/bundled",
    ...
    expect(roots).toEqual({
      ...
      workspace: path.join(homeDir, "ws", ".ultraclaw", "extensions"),
    });
  });
});
