...
import { clearPluginDiscoveryCache, discoverUltraClawPlugins } from "./discovery.js";
...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `ultraclaw-plugins-${randomUUID()}`);
  mkdirSafe(dir);
  tempDirs.push(dir);
  return dir;
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverUltraClawPlugins>[0],
) {
  return discoverUltraClawPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      ultraclaw: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverUltraClawPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".ultraclaw", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".ultraclaw", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverUltraClawPlugins({
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@ultraclaw/voice-call",
  ...
  it("normalizes bundled provider package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@ultraclaw/ollama-provider",
      extensions: ["./src/index.ts"],
    });
    fs.writeFileSync(
      path.join(globalExt, "src", "index.ts"),
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@ultraclaw/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@ultraclaw/escape-pack",
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverUltraClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        ULTRACLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    });
    ...
    const second = discoverUltraClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        ULTRACLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverUltraClawPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        ULTRACLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
