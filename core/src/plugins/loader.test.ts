...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "ultraclaw-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.ULTRACLAW_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "ultraclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.ULTRACLAW_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadUltraClawPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          ultraclaw: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.ULTRACLAW_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.ULTRACLAW_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadUltraClawPlugins>) {
...
function useNoBundledPlugins() {
  process.env.ULTRACLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadUltraClawPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function createEscapingEntryFixture(params: { id: string; sourceBody: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "ultraclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
        configSchema: EMPTY_PLUGIN_SCHEMA,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.ULTRACLAW_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.ULTRACLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadUltraClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadUltraClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadUltraClawPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadUltraClawPlugins(options);
  ...
  it("does not reuse cached bundled plugin registries across env changes", () => {
    ...
    const first = loadUltraClawPlugins({
      ...options,
      env: {
        ...process.env,
        ULTRACLAW_BUNDLED_PLUGINS_DIR: bundledA,
      },
    });
    const second = loadUltraClawPlugins({
      ...options,
      env: {
        ...process.env,
        ULTRACLAW_BUNDLED_PLUGINS_DIR: bundledB,
      },
    });
  ...
  it("does not reuse cached load-path plugin registries across env home changes", () => {
    ...
    const first = loadUltraClawPlugins({
      ...
      env: {
        ...process.env,
        HOME: homeA,
        ULTRACLAW_HOME: undefined,
        ULTRACLAW_STATE_DIR: stateDir,
        ULTRACLAW_BUNDLED_PLUGINS_DIR: bundledDir,
      },
    });
    const second = loadUltraClawPlugins({
      ...options,
  ...
  it("does not reuse cached registries when env-resolved install paths change", () => {
    useNoBundledPlugins();
    const ultraclawHome = makeTempDir();
    const ignoredHome = makeTempDir();
    const stateDir = makeTempDir();
    const pluginDir = path.join(ultraclawHome, "plugins", "tracked-install-cache");
    mkdirSafe(pluginDir);
    const plugin = writePlugin({
    ...
    const first = loadUltraClawPlugins({
      ...
      env: {
        ...process.env,
        ULTRACLAW_HOME: ultraclawHome,
        HOME: ignoredHome,
        ULTRACLAW_STATE_DIR: stateDir,
        CLAWDBOT_STATE_DIR: undefined,
        ULTRACLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
      },
    });
    ...
    const second = loadUltraClawPlugins(secondOptions);
    const third = loadUltraClawPlugins(secondOptions);
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadUltraClawPlugins({
      env: {
        ...
        HOME: homeDir,
        ULTRACLAW_HOME: undefined,
        ULTRACLAW_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers ULTRACLAW_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const ultraclawHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "ultraclaw-home-demo",
      dir: path.join(ultraclawHome, "plugins", "ultraclaw-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "ultraclaw-home-demo", register() {} };`,
    });
    ...
    const registry = loadUltraClawPlugins({
      ...
      config: {
        plugins: {
          allow: ["ultraclaw-home-demo"],
          entries: {
            "ultraclaw-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/ultraclaw-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "ultraclaw-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  it("skips importing bundled memory plugins that are disabled by memory slot", () => {
    ...
    fs.writeFileSync(
      path.join(memoryADir, "ultraclaw.plugin.json"),
      JSON.stringify(
        {
          id: "memory-a",
          kind: "memory",
    ...
    fs.writeFileSync(
      path.join(memoryBDir, "ultraclaw.plugin.json"),
      JSON.stringify(
        {
          id: "memory-b",
          kind: "memory",
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    const stateDir = makeTempDir();
    withEnv({ ULTRACLAW_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
  ...
  it("does not auto-load workspace-discovered plugins unless explicitly trusted", () => {
    ...
    const workspaceExtDir = path.join(workspaceDir, ".ultraclaw", "extensions", "workspace-helper");
  ...
  it("lets an explicitly trusted workspace plugin shadow a bundled plugin with the same id", () => {
    ...
    const workspaceDir = makeTempDir();
    const workspaceExtDir = path.join(workspaceDir, ".ultraclaw", "extensions", "shadowed");
    mkdirSafe(workspaceExtDir);
    writePlugin({
      id: "shadowed",
      body: `module.exports = { id: "shadowed", register() {} };`,
  ...
  it("does not warn about missing provenance for env-resolved load paths", () => {
    ...
    const ignoredHome = makeTempDir();
    const stateDir = makeTempDir();
    const pluginDir = path.join(ultraclawHome, "plugins", "tracked-load-path");
    mkdirSafe(pluginDir);
    const plugin = writePlugin({
  ...
  it("does not warn about missing provenance for env-resolved install paths", () => {
    ...
    const ignoredHome = makeTempDir();
    const stateDir = makeTempDir();
    const pluginDir = path.join(ultraclawHome, "plugins", "tracked-install-path");
    mkdirSafe(pluginDir);
    const plugin = writePlugin({
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("ultraclaw/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
    ...
    const script = `
      import { loadUltraClawPlugins } from ${JSON.stringify(loaderModuleUrl)};
      const registry = loadUltraClawPlugins({
        cache: false,
