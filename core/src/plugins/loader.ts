...
import { resolveUltraClawPackageRootSync } from "../infra/ultraclaw-root.js";
...
import { discoverUltraClawPlugins } from "./discovery.js";
...
function listPluginSdkExportedSubpaths(params: { modulePath?: string } = {}): string[] {
  const modulePath = params.modulePath ?? fileURLToPath(import.meta.url);
  const packageRoot = resolveUltraClawPackageRootSync({
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const subpath of listPluginSdkExportedSubpaths()) {
    ...
    if (resolved) {
      aliasMap[`ultraclaw/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: UltraClawPluginDefinition;
  register?: UltraClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as UltraClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as UltraClawPluginDefinition;
...
export function loadUltraClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverUltraClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "ultraclaw/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: UltraClawPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as UltraClawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
