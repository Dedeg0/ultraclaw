...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing ultraclaw.extensions; update the plugin package to include ultraclaw.extensions (for example ["./dist/index.js"]). See https://docs.ultraclaw.os/help/troubleshooting#plugin-install-fails-with-missing-ultraclaw-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_ULTRACLAW_EXTENSIONS: "missing_ultraclaw_extensions",
  EMPTY_ULTRACLAW_EXTENSIONS: "empty_ultraclaw_extensions",
...
function ensureUltraClawExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_ULTRACLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json ultraclaw.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_ULTRACLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureUltraClawExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from ultraclaw.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-ultraclaw"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "ultraclaw security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "ultraclaw security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(
  ...
  return await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "ultraclaw-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "ultraclaw-npm-pack-",
    spec,
    timeoutMs,
