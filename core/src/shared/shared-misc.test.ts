...
describe("shared/frontmatter", () => {
  ...
  test("resolveUltraClawManifestBlock parses JSON5 metadata and picks ultraclaw block", () => {
    const frontmatter = {
      metadata: "{ ultraclaw: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveUltraClawManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveUltraClawManifestBlock returns undefined for invalid input", () => {
    expect(resolveUltraClawManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveUltraClawManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveUltraClawManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
