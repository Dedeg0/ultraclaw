...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): UltraClawConfig {
  return {
    messages: {
      tts: {
        provider: "openai",
...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.ULTRACLAW_TTS_PREFS;
      process.env.ULTRACLAW_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      const originalFetch = globalThis.fetch;
      const fetchMock = vi.fn(async () => ({
        ok: true,
      ...
      try {
        ...
        globalThis.fetch = originalFetch;
        process.env.ULTRACLAW_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: UltraClawConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
