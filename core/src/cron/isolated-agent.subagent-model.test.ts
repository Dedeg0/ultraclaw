...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "ultraclaw-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".ultraclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<UltraClawConfig> = {},
): UltraClawConfig {
  const base: UltraClawConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<UltraClawConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<UltraClawConfig>,
