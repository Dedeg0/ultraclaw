...
const { makeStorePath } = createCronStoreHarness({ prefix: "ultraclaw-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ULTRACLAW_HOME for tilde expansion", () => {
    vi.stubEnv("ULTRACLAW_HOME", "/srv/ultraclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/ultraclaw-home", "cron", "jobs.json"));
  });
});
