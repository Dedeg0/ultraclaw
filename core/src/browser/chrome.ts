...
export function resolveUltraClawUserDataDir(profileName = DEFAULT_ULTRACLAW_BROWSER_PROFILE_NAME) {
...
export async function launchUltraClawChrome(
  ...
  const userDataDir = resolveUltraClawUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_ULTRACLAW_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateUltraClawProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 ultraclaw browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`ultraclaw browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`ultraclaw browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 ultraclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopUltraClawChrome(
