...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("ultraclaw");
    expect(profile?.driver).toBe("ultraclaw");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const ultraclaw = resolveProfile(resolved, "ultraclaw");
    expect(ultraclaw?.driver).toBe("ultraclaw");
    expect(ultraclaw?.cdpPort).toBe(18800);
    expect(ultraclaw?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from ULTRACLAW_GATEWAY_PORT when unset", () => {
    withEnv({ ULTRACLAW_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const ultraclaw = resolveProfile(resolved, "ultraclaw");
      expect(ultraclaw?.cdpPort).toBe(19012);
      expect(ultraclaw?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ ULTRACLAW_GATEWAY_PORT: undefined }, () => {
      ...
      const ultraclaw = resolveProfile(resolved, "ultraclaw");
      expect(ultraclaw?.cdpPort).toBe(19022);
      expect(ultraclaw?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(ultraclaw?.cdpPort).toBe(19000);
    expect(ultraclaw?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "ultraclaw");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        ultraclaw: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("ultraclaw");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to ultraclaw profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps ultraclaw default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("ultraclaw");
    ...
    it("keeps ultraclaw default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("ultraclaw");
    ...
    it("keeps ultraclaw default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
