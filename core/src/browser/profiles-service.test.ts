...
vi.mock("./chrome.js", () => ({
  resolveUltraClawUserDataDir: vi.fn(() => "/tmp/ultraclaw-test/ultraclaw/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "ultraclaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveUltraClawUserDataDir).mockReturnValue(userDataDir);
