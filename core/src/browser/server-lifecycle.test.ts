...
const { stopUltraClawChromeMock, stopChromeExtensionRelayServerMock } = vi.hoisted(() => ({
  stopUltraClawChromeMock: vi.fn(async () => {}),
...
vi.mock("./chrome.js", () => ({
  stopUltraClawChrome: stopUltraClawChromeMock,
}));
...
describe("ensureExtensionRelayForProfiles", () => {
  ...
  it("starts relay only for extension profiles", async () => {
    resolveProfileMock.mockImplementation((_resolved: unknown, name: string) => {
      ...
      return { driver: "ultraclaw", cdpUrl: "http://127.0.0.1:18889" };
    });
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopUltraClawChromeMock.mockClear();
    stopChromeExtensionRelayServerMock.mockClear();
  });

  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["ultraclaw", "chrome"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      ultraclaw: vi.fn(async () => {}),
      chrome: vi.fn(async () => {
    ...
    expect(stopMap.ultraclaw).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopUltraClawChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("ultraclaw browser stop failed: Error: oops");
  });
});
