...
import {
  probeAuthenticatedUltraClawRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedUltraClawRelay({
    baseUrl,
    relayAuthHeader: "x-ultraclaw-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated ultraclaw relay probe responses", async () => {
