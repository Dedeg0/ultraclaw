...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".ultraclaw-profile-decorated");
}
 ...
 */
export function decorateUltraClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ULTRACLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ULTRACLAW_BROWSER_COLOR).toUpperCase();
