...
function localUltraClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "ultraclaw",
...
function createLocalUltraClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localUltraClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveUltraClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "ultraclaw-reset-"));
    const profileDir = path.join(tempRoot, "ultraclaw");
    ...
    const ops = createLocalUltraClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveUltraClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "ultraclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "ultraclaw");
    fs.mkdirSync(profileDir, { recursive: true });
