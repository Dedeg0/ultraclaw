  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          ultraclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ULTRACLAW_GATEWAY_PORT;
    process.env.ULTRACLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ULTRACLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ULTRACLAW_GATEWAY_PASSWORD;
