import type { BrowserConfig, BrowserProfileConfig, UltraClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_ULTRACLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_ULTRACLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "ultraclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ULTRACLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ULTRACLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an UltraClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for ultraclaw/ultraclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: UltraClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ULTRACLAW_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_ULTRACLAW_BROWSER_PROFILE_NAME]
        ? DEFAULT_ULTRACLAW_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "ultraclaw";
