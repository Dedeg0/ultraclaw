...
homepage: https://docs.ultraclaw.os/automation/hooks#command-logger
metadata:
  {
    "ultraclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.ultraclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.ultraclaw/logs/commands.log ~/.ultraclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/ultraclaw`:
   ```
   /home/username/.ultraclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
