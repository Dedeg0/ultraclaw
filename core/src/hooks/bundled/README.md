...
This directory contains hooks that ship with UltraClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.ultraclaw/ultraclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'ultraclaw.*gateway' && pnpm ultraclaw gateway`
3. Enable the hook: `ultraclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.ultraclaw.os/automation/hooks
