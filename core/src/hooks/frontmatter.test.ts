...
`;
    ...
    expect(parsed.ultraclaw.emoji).toBe("💾");
    expect(parsed.ultraclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.ultraclaw.emoji).toBe("📝");
    expect(parsed.ultraclaw.events).toEqual(["command"]);
    expect(parsed.ultraclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.ultraclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"ultraclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"ultraclaw": {"events": ["test"]}}');
  });
...
describe("resolveUltraClawMetadata", () => {
  it("extracts ultraclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveUltraClawMetadata(frontmatter);
  ...
  it("returns undefined when ultraclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        ultraclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with UltraClaw" },
            { id: "npm", kind: "npm", package: "@ultraclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@ultraclaw/hook");
  });
    ...
    "ultraclaw":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with UltraClaw" }],
      },
  }
---
...
`;
    ...
    const ultraclaw = resolveUltraClawMetadata(frontmatter);
    expect(ultraclaw).toBeDefined();
    expect(ultraclaw?.emoji).toBe("💾");
    expect(ultraclaw?.events).toEqual(["command:new", "command:reset"]);
    expect(ultraclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(ultraclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const ultraclaw = resolveUltraClawMetadata(frontmatter);
    expect(ultraclaw?.emoji).toBe("disk");
    expect(ultraclaw?.events).toEqual(["command:new"]);
  });
});
