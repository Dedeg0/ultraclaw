...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        ULTRACLAW_BASE_ENV: "base",
        ULTRACLAW_TO_REMOVE: undefined,
      },
      env: {
        ULTRACLAW_TEST_ENV: "ok",
      },
    });

    expect(resolved.ULTRACLAW_BASE_ENV).toBe("base");
    expect(resolved.ULTRACLAW_TEST_ENV).toBe("ok");
    expect(resolved.ULTRACLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.ULTRACLAW_CLI).toBe(ULTRACLAW_CLI_ENV_VALUE);
  });
