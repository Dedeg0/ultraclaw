...
describe("agent components", () => {
  const createCfg = (): UltraClawConfig => ({}) as UltraClawConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): UltraClawConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/ultraclaw-sessions-test.json");
  });
