...
function resolveVoiceTtsConfig(params: { cfg: UltraClawConfig; override?: TtsConfig }): {
  cfg: UltraClawConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredUltraClawTmpDir(), "discord-voice-"));
