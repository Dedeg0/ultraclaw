  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/ultraclaw",
      },
    },
    session: { store: "/tmp/ultraclaw-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bultraclaw\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "ultraclaw: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "UltraClaw" },
          },
        },
        eventPatch: {
