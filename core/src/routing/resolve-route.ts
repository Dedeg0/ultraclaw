...
function listAgents(cfg: UltraClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: UltraClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<UltraClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: UltraClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: UltraClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: UltraClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<UltraClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  UltraClawConfig,
  {
    bindingsRef: UltraClawConfig["bindings"];
    agentsRef: UltraClawConfig["agents"];
    sessionRef: UltraClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: UltraClawConfig): Map<string, ResolvedAgentRoute> {
