...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: UltraClawConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: UltraClawConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/ultraclaw-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<UltraClawConfig["bindings"]>[number];
