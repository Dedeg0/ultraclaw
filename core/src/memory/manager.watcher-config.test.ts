...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-memory-watch-"));
  ...
  function createWatcherConfig(
    overrides?: Partial<NonNullable<UltraClawConfig["agents"]>["defaults"]["memorySearch"]>,
  ): UltraClawConfig {
    return {
      agents: {
        defaults: {
  ...
  async function expectWatcherManager(cfg: UltraClawConfig) {
