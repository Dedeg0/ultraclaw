...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set ULTRACLAW_GATEWAY_TOKEN/ULTRACLAW_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "ultraclaw-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.ULTRACLAW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.ULTRACLAW_GATEWAY_PASSWORD);
