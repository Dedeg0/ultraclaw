...
describe("resolveInitialTuiAgentId", () => {
  const cfg: UltraClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/ultraclaw" },
        { id: "ops", workspace: "/tmp/ultraclaw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/ultraclaw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run ultraclaw devices list");
    expect(state.pairingHint).toContain("ultraclaw devices list");
  });
