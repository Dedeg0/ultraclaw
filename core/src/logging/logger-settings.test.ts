...
let originalUltraClawLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.ULTRACLAW_TEST_FILE_LOG;
  originalUltraClawLogLevel = process.env.ULTRACLAW_LOG_LEVEL;
  delete process.env.ULTRACLAW_TEST_FILE_LOG;
  delete process.env.ULTRACLAW_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.ULTRACLAW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalUltraClawLogLevel === undefined) {
    delete process.env.ULTRACLAW_LOG_LEVEL;
  } else {
    process.env.ULTRACLAW_LOG_LEVEL = originalUltraClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
