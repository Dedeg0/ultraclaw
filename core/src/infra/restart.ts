...
export function triggerUltraClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.ULTRACLAW_SYSTEMD_UNIT,
      process.env.ULTRACLAW_PROFILE,
    );
  ...
  const label =
    process.env.ULTRACLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ULTRACLAW_PROFILE);
