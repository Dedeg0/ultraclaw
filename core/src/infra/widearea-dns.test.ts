...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "ultraclaw.internal", expected: "ultraclaw.internal." },
    { value: "ultraclaw.internal.", expected: "ultraclaw.internal." },
    { value: "  ultraclaw.internal  ", expected: "ultraclaw.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { ULTRACLAW_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { ULTRACLAW_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("ultraclaw.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "ultraclaw.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  ...
  it("includes gateway TLS TXT fields and trims display metadata", () => {
    ...
    expect(txt).toContain(`$ORIGIN ultraclaw.internal.`);
    expect(txt).toContain(`studio-london IN A 100.123.224.76`);
    expect(txt).toContain(`studio-london._ultraclaw-gw._tcp IN TXT`);
    expect(txt).toContain(`displayName=Mac Studio (UltraClaw)`);
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("ultraclaw.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("ultraclaw.internal."),
