...
describe("resolveEffectiveHomeDir", () => {
  it("prefers ULTRACLAW_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/ultraclaw-home"),
    );
  });
  ...
  it("expands ULTRACLAW_HOME when set to ~", () => {
    const env = {
      ULTRACLAW_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    ...
    expect(value).toBe(`${path.resolve("/srv/ultraclaw-home")}/x`);
  });
