...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/ultraclaw-state", "devices")).toEqual({
      dir: "/tmp/ultraclaw-state/devices",
      pendingPath: "/tmp/ultraclaw-state/devices/pending.json",
      pairedPath: "/tmp/ultraclaw-state/devices/paired.json",
    });
  });
