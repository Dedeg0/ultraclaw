...
const PRIMARY_PACKAGE_NAME = "ultraclaw";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "UltraClaw", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.ULTRACLAW_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.ULTRACLAW_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
