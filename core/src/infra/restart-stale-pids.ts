 ...
 * returning control to the caller (typically `triggerUltraClawRestart` →
...
/**
 * Parse ultraclaw gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("ultraclaw")) {
 ...
 * Returns only PIDs that belong to ultraclaw gateway processes (not the current process).
 */
