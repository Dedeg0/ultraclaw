...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __ultraclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__ultraclawDiagnosticEventsState) {
    globalStore.__ultraclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__ultraclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: UltraClawConfig): boolean {
