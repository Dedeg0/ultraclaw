const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "ULTRACLAW_LAUNCHD_LABEL"],
  systemd: ["ULTRACLAW_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["ULTRACLAW_WINDOWS_TASK_NAME"],
} as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.ULTRACLAW_SERVICE_MARKER?.trim();
    const serviceKind = env.ULTRACLAW_SERVICE_KIND?.trim();
