...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ULTRACLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.ULTRACLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveUltraClawPackageRoot } = await import("./ultraclaw-root.js"));
    ...
    vi.mocked(resolveUltraClawPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveUltraClawPackageRoot).mockResolvedValue("/opt/ultraclaw");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/ultraclaw/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
