...
function candidateBinDirs(opts: EnsureUltraClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `ultraclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "ultraclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.ULTRACLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "ultraclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `ultraclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureUltraClawCliOnPath(opts: EnsureUltraClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ULTRACLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
