...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred UltraClaw tmp root", async () => {
    const tmpRoot = resolvePreferredUltraClawTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideUltraClaw = path.join(os.tmpdir(), "outside-ultraclaw", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
