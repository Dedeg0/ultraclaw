...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: UltraClawConfig | null = null;
  ...
  get(key: string, cfg: UltraClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: UltraClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: UltraClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: UltraClawConfig): void {
