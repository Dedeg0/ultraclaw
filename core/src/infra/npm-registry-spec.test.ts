...
describe("npm registry spec validation", () => {
  ...
  it("rejects unsupported registry protocols and malformed selectors", () => {
    ...
    expect(validateRegistryNpmSpec("git+ssh://github.com/ultraclaw/ultraclaw")).toContain(
      "URLs are not allowed",
    );
    expect(validateRegistryNpmSpec("@ultraclaw/voice-call@")).toContain(
      "missing version/tag after @",
    );
    expect(validateRegistryNpmSpec("@ultraclaw/voice-call@../beta")).toContain(
      "invalid version/tag",
    );
  });
});
...
describe("npm registry spec parsing helpers", () => {
  it("parses bare, tag, and exact prerelease specs", () => {
    expect(parseRegistryNpmSpec("@ultraclaw/voice-call")).toEqual({
      name: "@ultraclaw/voice-call",
      raw: "@ultraclaw/voice-call",
      selectorKind: "none",
      selectorIsPrerelease: false,
    });
    expect(parseRegistryNpmSpec("@ultraclaw/voice-call@beta")).toEqual({
      name: "@ultraclaw/voice-call",
      raw: "@ultraclaw/voice-call@beta",
      selector: "beta",
      selectorKind: "tag",
      selectorIsPrerelease: false,
    });
    expect(parseRegistryNpmSpec("@ultraclaw/voice-call@1.2.3-beta.1")).toEqual({
      name: "@ultraclaw/voice-call",
      raw: "@ultraclaw/voice-call@1.2.3-beta.1",
...
describe("npm prerelease resolution policy", () => {
  ...
  it("allows stable resolutions even for bare and latest specs", () => {
    const bareSpec = parseRegistryNpmSpec("@ultraclaw/voice-call");
    const latestSpec = parseRegistryNpmSpec("@ultraclaw/voice-call@latest");
  ...
  it("formats prerelease resolution guidance based on selector intent", () => {
    ...
    ).toContain(`Use "@ultraclaw/voice-call@beta"`);
    expect(
      formatPrereleaseResolutionError({
        spec: tagSpec!,
        resolvedVersion: "1.2.3-rc.1",
