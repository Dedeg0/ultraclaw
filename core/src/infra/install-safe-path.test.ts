...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@ultraclaw/matrix", expected: "matrix" },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@ultraclaw__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-install-safe-"));
    try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-install-safe-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-install-safe-outside-"));
      try {
