...
describe("system-presence version fallback", () => {
  ...
  it("uses runtime VERSION when ULTRACLAW_VERSION is not set", async () => {
  ...
  it("prefers ULTRACLAW_VERSION over runtime VERSION", async () => {
    await expectSelfVersion(
      {
  ...
  it("still prefers runtime VERSION over ULTRACLAW_SERVICE_VERSION when ULTRACLAW_VERSION is blank", async () => {
    await expectSelfVersion(
      {
        ULTRACLAW_VERSION: " ",
        ULTRACLAW_SERVICE_VERSION: "2.4.6-service",
  ...
  it("uses runtime VERSION when ULTRACLAW_VERSION and ULTRACLAW_SERVICE_VERSION are blank", async () => {
    await expectSelfVersion(
      {
        ULTRACLAW_VERSION: " ",
        ULTRACLAW_SERVICE_VERSION: "\t",
