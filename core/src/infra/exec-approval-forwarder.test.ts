...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): UltraClawConfig {
  return {
...
describe("exec approval forwarder", () => {
  ...
  it("prefers turn-source routing over stale session last route", async () => {
    vi.useFakeTimers();
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "ultraclaw-exec-approval-forwarder-test-"));
    try {
