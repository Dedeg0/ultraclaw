...
import { POSIX_ULTRACLAW_TMP_DIR, resolvePreferredUltraClawTmpDir } from "./tmp-ultraclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredUltraClawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `ultraclaw-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredUltraClawTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_ULTRACLAW_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredUltraClawTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredUltraClawTmpDir", () => {
  it("prefers /tmp/ultraclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_ULTRACLAW_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/ultraclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_ULTRACLAW_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/ultraclaw when /tmp/ultraclaw is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/ultraclaw when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/ultraclaw is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/ultraclaw is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/ultraclaw is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback UltraClaw temp dir/);
  });
