...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ULTRACLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*ultraclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "ultraclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "ultraclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "ultraclaw-gw");
  ...
  records.push(`_ultraclaw-gw._tcp IN PTR ${instanceLabel}._ultraclaw-gw._tcp`);
  records.push(`${instanceLabel}._ultraclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._ultraclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; ultraclaw-content-hash: ${contentHash}\n${contentBody}`;
}
