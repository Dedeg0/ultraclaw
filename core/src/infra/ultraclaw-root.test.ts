...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__ultraclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "ultraclaw-root");
...
describe("resolveUltraClawPackageRoot", () => {
  let resolveUltraClawPackageRoot: typeof import("./ultraclaw-root.js").resolveUltraClawPackageRoot;
  let resolveUltraClawPackageRootSync: typeof import("./ultraclaw-root.js").resolveUltraClawPackageRootSync;

  beforeAll(async () => {
    ({ resolveUltraClawPackageRoot, resolveUltraClawPackageRootSync } =
      await import("./ultraclaw-root.js"));
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "ultraclaw");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "ultraclaw.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "ultraclaw" }));

    expect(resolveUltraClawPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveUltraClawPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveUltraClawPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
    ).resolves.toBe(pkgRoot);
  });

  it("returns null for non-ultraclaw package roots", async () => {
    const pkgRoot = fx("not-ultraclaw");
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveUltraClawPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveUltraClawPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
