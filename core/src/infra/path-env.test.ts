...
let ensureUltraClawCliOnPath: typeof import("./path-env.js").ensureUltraClawCliOnPath;

describe("ensureUltraClawCliOnPath", () => {
  const envKeys = [
    "PATH",
    "ULTRACLAW_PATH_BOOTSTRAPPED",
    "ULTRACLAW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureUltraClawCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling ultraclaw exists", () => {
    const tmp = abs("/tmp/ultraclaw-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "ultraclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.ULTRACLAW_PATH_BOOTSTRAPPED;

    ensureUltraClawCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.ULTRACLAW_PATH_BOOTSTRAPPED = "1";
    ensureUltraClawCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/ultraclaw-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "ultraclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/ultraclaw-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "ultraclaw");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "ultraclaw");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/ultraclaw-path/case-linuxbrew");
