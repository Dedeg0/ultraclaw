...
describe("ensureUltraClawCliOnPath", () => {
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/ultraclaw-path/${name}`);
  ...
  function bootstrapPath(params: {
    ...
    ensureUltraClawCliOnPath(params);
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.ULTRACLAW_PATH_BOOTSTRAPPED;
      delete process.env.ULTRACLAW_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.ULTRACLAW_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
