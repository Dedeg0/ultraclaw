...
const DEFAULT_PACKAGE_NAME = "ultraclaw";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["ultraclaw", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "ultraclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("ultraclaw doctor", doctorArgv, gitRoot, { ULTRACLAW_UPDATE_IN_PROGRESS: "1" }),
    );
