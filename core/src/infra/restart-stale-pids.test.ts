...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses ultraclaw-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "ultraclaw-gateway" },
          { pid: process.pid, cmd: "ultraclaw-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'ultraclaw'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple ultraclaw pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "ultraclaw-gateway" },
          { pid: pid2, cmd: "ultraclaw-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18790);
    ...
    it("returns [] when status 0 but only non-ultraclaw pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks ultraclaw processes — non-ultraclaw listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 1) {
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: stalePid, cmd: "ultraclaw-gateway" }]),
            stderr: "",
          };
        }
    ...
    it("lsof status 1 with non-empty ultraclaw stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty ultraclaw stdout must not be treated as port-free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 2) {
          // status 1 + ultraclaw pid in stdout — container-restricted lsof reports partial results
          return {
            error: null,
            status: 1,
            stdout: lsofOutput([{ pid: stalePid, cmd: "ultraclaw-gateway" }]),
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'ultraclaw'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("ultraclaw")
      // inside the mid-loop flush: a non-ultraclaw cmd between two entries must not
      // be pushed, but the following ultraclaw entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-ultraclaw entry first, then ultraclaw entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid ultraclaw entry must still be found.
      const stdout = `p0\ncultraclaw-gateway\np${stalePid}\ncultraclaw-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18790);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero ultraclaw pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-ultraclaw stdout (line 145)", () => {
    it("treats status 1 + non-ultraclaw stdout as port-free (not an ultraclaw process)", () => {
      // status 1 + non-empty stdout where no ultraclaw pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill ultraclaw pids) it is effectively free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        // status 1 + non-ultraclaw output — should be treated as free:true for our purposes
        return {
          error: null,
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no ultraclaw pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
