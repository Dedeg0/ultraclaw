...
describe("ports-format", () => {
  it("classifies listeners across gateway, ssh, and unknown command lines", () => {
    const cases = [
      {
        listener: { commandLine: "ssh -N -L 18790:127.0.0.1:18790 user@host" },
        expected: "ssh",
      },
      {
      ...
      {
        listener: { commandLine: "node /Users/me/Projects/ultraclaw/dist/entry.js gateway" },
        expected: "gateway",
      },
      {
        listener: { commandLine: "python -m http.server 18790" },
        expected: "unknown",
      },
    ] as const;
    ...
    for (const testCase of cases) {
      expect(
        classifyPortListener(testCase.listener, 18790),
  ...
  it("builds ordered hints for mixed listener kinds and multiplicity", () => {
    expect(
      buildPortHints(
        [
          { commandLine: "node dist/index.js ultraclaw gateway" },
  ...
  it("formats listeners with pid, user, command, and address fallbacks", () => {
    ...
    expect(formatPortListener({ command: "ssh", address: "127.0.0.1:18790" })).toBe(
      "pid ?: ssh (127.0.0.1:18790)",
    );
