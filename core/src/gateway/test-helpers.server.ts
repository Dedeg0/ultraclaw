...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "ULTRACLAW_SKIP_BROWSER_CONTROL_SERVER",
  "ULTRACLAW_SKIP_GMAIL_WATCHER",
  "ULTRACLAW_SKIP_CANVAS_HOST",
  "ULTRACLAW_BUNDLED_PLUGINS_DIR",
  "ULTRACLAW_SKIP_CHANNELS",
  "ULTRACLAW_SKIP_PROVIDERS",
  "ULTRACLAW_SKIP_CRON",
  "ULTRACLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ULTRACLAW_STATE_DIR = path.join(tempHome, ".ultraclaw");
  delete process.env.ULTRACLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.ULTRACLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.ULTRACLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.ULTRACLAW_SKIP_CANVAS_HOST = "1";
  process.env.ULTRACLAW_SKIP_CHANNELS = "1";
  process.env.ULTRACLAW_SKIP_PROVIDERS = "1";
  process.env.ULTRACLAW_SKIP_CRON = "1";
  process.env.ULTRACLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.ULTRACLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "ultraclaw-test-no-bundled-extensions")
    : "ultraclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".ultraclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".ultraclaw-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__ultraclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__ultraclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["ULTRACLAW_GATEWAY_TOKEN"]);
  const prev = process.env.ULTRACLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ULTRACLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.ULTRACLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
