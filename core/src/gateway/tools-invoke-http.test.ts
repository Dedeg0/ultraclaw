...
let lastCreateUltraClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/ultraclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateUltraClawToolsContext?.agentTo,
          agentThreadId: lastCreateUltraClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createUltraClawTools: (ctx: Record<string, unknown>) => {
      lastCreateUltraClawToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateUltraClawToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateUltraClawToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-ultraclaw-message-to": "channel:24514",
        "x-ultraclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
