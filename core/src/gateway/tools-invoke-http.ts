...
import { createUltraClawTools } from "../agents/ultraclaw-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-ultraclaw-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-ultraclaw-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-ultraclaw-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-ultraclaw-thread-id")?.trim() || undefined;
  ...
  const allTools = createUltraClawTools({
