...
type GetReplyFromConfigFn = (
  ...
  configOverride?: UltraClawConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `ultraclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.ULTRACLAW_CONFIG_PATH = path.join(root, "ultraclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "ultraclaw.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "ultraclaw-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadUltraClawPlugins: () => pluginRegistryState.registry,
  };
});
