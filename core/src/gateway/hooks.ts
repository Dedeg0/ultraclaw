...
export function resolveHooksConfig(cfg: UltraClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: UltraClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-ultraclaw-token"] === "string"
      ? req.headers["x-ultraclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-ultraclaw-idempotency-key"]) ||
