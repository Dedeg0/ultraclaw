...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.ULTRACLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.ULTRACLAW_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.ULTRACLAW_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    prevGeminiApiKey = process.env.GEMINI_API_KEY;
    process.env.ULTRACLAW_SKIP_CHANNELS = "0";
    delete process.env.ULTRACLAW_SKIP_GMAIL_WATCHER;
    delete process.env.ULTRACLAW_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.ULTRACLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.ULTRACLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ULTRACLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.ULTRACLAW_SKIP_PROVIDERS;
    } else {
      process.env.ULTRACLAW_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeEnvRefConfig() {
    const configPath = process.env.ULTRACLAW_CONFIG_PATH;
    if (!configPath) {
      throw new Error("ULTRACLAW_CONFIG_PATH is not set");
    }
    await fs.writeFile(
      configPath,
      `${JSON.stringify(
  ...
  async function writeAuthProfileEnvRefStore() {
    const stateDir = process.env.ULTRACLAW_STATE_DIR;
    if (!stateDir) {
      throw new Error("ULTRACLAW_STATE_DIR is not set");
    }
      ...
      `${JSON.stringify(
        {
          ...
          profiles: {
            missing: {
              type: "api_key",
              provider: "openai",
              keyRef: { source: "env", provider: "default", id: "MISSING_ULTRACLAW_AUTH_REF" },
            },
          },
  ...
  it("fails startup when auth-profile secret refs are unresolved", async () => {
    ...
    delete process.env.MISSING_ULTRACLAW_AUTH_REF;
    try {
      await expect(withGatewayServer(async () => {})).rejects.toThrow(
        'Environment variable "MISSING_ULTRACLAW_AUTH_REF" is missing or empty.',
      );
    } finally {
...
`,
    ...
    const previousGatewayTokenEnv = process.env.ULTRACLAW_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.ULTRACLAW_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
      envSnapshot.restore();
      ws.close();
      await server.close();
