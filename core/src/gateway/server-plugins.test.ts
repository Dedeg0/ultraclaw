...
const loadUltraClawPlugins = vi.hoisted(() => vi.fn());
...
function createSubagentRuntime(serverPlugins: ServerPluginsModule): PluginRuntime["subagent"] {
  ...
  loadUltraClawPlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadUltraClawPlugins.mock.calls.at(-1)?.[0] as
...
beforeEach(() => {
  loadUltraClawPlugins.mockReset();
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadUltraClawPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("provides subagent runtime with sessions.get method aliases", async () => {
    ...
    const call = loadUltraClawPlugins.mock.calls.at(-1)?.[0];
