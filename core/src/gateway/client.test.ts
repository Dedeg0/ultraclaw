...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("ultraclaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with ULTRACLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with ULTRACLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.ULTRACLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://ultraclaw-gateway.ai:18790",
      onConnectError,
    });
