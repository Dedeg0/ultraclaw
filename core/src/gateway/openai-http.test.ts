    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "ultraclaw", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-ultraclaw-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-ultraclaw-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "ultraclaw", messages: [{ role: "user", content: "hi" }] },
          {
            "x-ultraclaw-agent-id": "beta",
            "x-ultraclaw-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from UltraClaw.");
      }
