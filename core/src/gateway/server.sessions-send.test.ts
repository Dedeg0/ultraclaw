...
const { createUltraClawTools } = await import("../agents/ultraclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createUltraClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createUltraClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ULTRACLAW_GATEWAY_PORT", "ULTRACLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.ULTRACLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.ULTRACLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("ULTRACLAW_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createUltraClawTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
