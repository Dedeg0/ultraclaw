...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveUltraClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("ultraclaw models auth setup-token")}\` or \`${formatCliCommand("ultraclaw configure")}\`.`
          : `Run \`${formatCliCommand("ultraclaw configure")}\` or set an API key env var.`;
