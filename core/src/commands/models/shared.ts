...
export async function loadValidConfigOrThrow(): Promise<UltraClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: UltraClawConfig) => UltraClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: UltraClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: UltraClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("ultraclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
