...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveUltraClawAgentDir: vi.fn().mockReturnValue("/tmp/ultraclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/ultraclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/ultraclaw-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/ultraclaw-dev/ultraclaw.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveUltraClawAgentDir: mocks.resolveUltraClawAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/ultraclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveUltraClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/ultraclaw-dev/ultraclaw.json");
    expect(payload.auth.storePath).toBe("/tmp/ultraclaw-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/ultraclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/ultraclaw-agent-custom");
