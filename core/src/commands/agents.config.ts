...
function resolveAgentName(cfg: UltraClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: UltraClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: UltraClawConfig): AgentSummary[] {
