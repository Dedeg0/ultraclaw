...
type PromptDefaultModelResult = { model?: string; config?: UltraClawConfig };
...
function resolveConfiguredModelRaw(cfg: UltraClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: UltraClawConfig): string[] {
...
export function applyPrimaryModel(cfg: UltraClawConfig, model: string): UltraClawConfig {
...
export function applyModelAllowlist(cfg: UltraClawConfig, models: string[]): UltraClawConfig {
  const defaults = cfg.agents?.defaults;
