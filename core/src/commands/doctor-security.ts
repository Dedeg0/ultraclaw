...
import type { UltraClawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: UltraClawConfig): string[] {
...
export async function noteSecurityWarnings(cfg: UltraClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("ultraclaw security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.ultraclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("ultraclaw approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.ultraclaw.os/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("ultraclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("ultraclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("ultraclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("ultraclaw config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('ultraclaw config set session.dmScope "per-channel-peer"') +
