...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under ULTRACLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("ultraclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-oauth-sync-"));
    ...
    process.env.ULTRACLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-oauth-nosync-"));
    process.env.ULTRACLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside ULTRACLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-oauth-external-"));
    process.env.ULTRACLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* ULTRACLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ULTRACLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("ultraclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
