...
vi.mock("../daemon/service-audit.js", () => ({
  ...
  readEmbeddedGatewayToken: (
    ...
    command?.environmentValueSources?.ULTRACLAW_GATEWAY_TOKEN === "file"
      ? undefined
      : command?.environment?.ULTRACLAW_GATEWAY_TOKEN?.trim() || undefined,
  SERVICE_AUDIT_CODES: {
...
function setupGatewayTokenRepairScenario() {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      ULTRACLAW_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service ULTRACLAW_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    ...
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: UltraClawConfig, env) => {
      const configToken =
        typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
      const envToken = env.ULTRACLAW_GATEWAY_TOKEN?.trim() || undefined;
  ...
  it("uses ULTRACLAW_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ ULTRACLAW_GATEWAY_TOKEN: "env-token" }, async () => {
      setupGatewayTokenRepairScenario();
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/Library/pnpm/global/5/node_modules/ultraclaw/dist/index.js",
        "gateway",
        "--port",
        "18790",
      ],
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/ultraclaw@2026.3.12/node_modules/ultraclaw/dist/index.js",
        "gateway",
        "--port",
        "18790",
      ],
    ...
    fsMocks.realpath.mockImplementation(async (value: string) => {
      if (value.includes("/global/5/node_modules/ultraclaw/")) {
        return value.replace(
          "/global/5/node_modules/ultraclaw/",
          "/global/5/node_modules/.pnpm/ultraclaw@2026.3.12/node_modules/ultraclaw/",
        );
      }
      return value;
    });
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/opt/ultraclaw/../ultraclaw/dist/index.js",
        "gateway",
        "--port",
        "18790",
      ],
  ...
  it("still flags entrypoint mismatch when canonicalized paths differ", async () => {
    mocks.readCommand.mockResolvedValue({
      programArguments: [
        "/usr/bin/node",
        "/Users/test/.nvm/versions/node/v22.0.0/lib/node_modules/ultraclaw/dist/index.js",
        "gateway",
        "--port",
        "18790",
      ],
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            ULTRACLAW_GATEWAY_TOKEN: "env-file-token",
          },
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing UltraClaw gateway next.",
    );
  });
});
