...
type EnvSnapshot = {
  HOME?: string;
  ULTRACLAW_HOME?: string;
  ULTRACLAW_STATE_DIR?: string;
  ULTRACLAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    ULTRACLAW_HOME: process.env.ULTRACLAW_HOME,
    ULTRACLAW_STATE_DIR: process.env.ULTRACLAW_STATE_DIR,
    ULTRACLAW_OAUTH_DIR: process.env.ULTRACLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: UltraClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: UltraClawConfig) {
...
async function runStateIntegrityText(cfg: UltraClawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "ultraclaw-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.ULTRACLAW_HOME = tempHome;
    process.env.ULTRACLAW_STATE_DIR = path.join(tempHome, ".ultraclaw");
    delete process.env.ULTRACLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.ULTRACLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when ULTRACLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.ULTRACLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: UltraClawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints ultraclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: UltraClawConfig = {};
    ...
    expect(text).toMatch(/ultraclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/ultraclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /ultraclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
