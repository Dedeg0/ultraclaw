...
describe("readServiceStatusSummary", () => {
  it("marks UltraClaw-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["ultraclaw", "gateway", "run"] })),
    ...
    expect(summary.managedByUltraClaw).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByUltraClaw).toBe(false);
