...
import { resolveUltraClawPackageRoot } from "../infra/ultraclaw-root.js";
...
function resolveMode(cfg: UltraClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("UltraClaw doctor");

  const root = await resolveUltraClawPackageRoot({
  ...
  let cfg: UltraClawConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("ultraclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("ultraclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("ultraclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("ultraclaw config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("ultraclaw config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("ultraclaw doctor --fix")}" to apply changes.`);
  }
