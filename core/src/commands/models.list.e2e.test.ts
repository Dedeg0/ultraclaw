...
const ensureUltraClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveUltraClawAgentDir = vi.fn().mockReturnValue("/tmp/ultraclaw-agent");
...
vi.mock("../agents/models-config.js", () => ({
  ensureUltraClawModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveUltraClawAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureUltraClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureUltraClawModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureUltraClawModelsJson).toHaveBeenCalled();
    expect(ensureUltraClawModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
