...
function pruneAuthProfiles(
  cfg: UltraClawConfig,
  profileIds: Set<string>,
): { next: UltraClawConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("ultraclaw models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  ...
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("ultraclaw models auth setup-token")} or ${formatCliCommand(
      "ultraclaw configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "ultraclaw models auth login --provider openai-codex",
    )} or ${formatCliCommand("ultraclaw configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("ultraclaw configure")}\` or \`${formatCliCommand("ultraclaw onboard")}\`.`;
}
