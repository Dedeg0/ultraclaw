...
export function normalizeCompatibilityConfigValues(cfg: UltraClawConfig): {
  config: UltraClawConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: UltraClawConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as UltraClawConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as UltraClawConfig["browser"],
    };
    changes.push(
