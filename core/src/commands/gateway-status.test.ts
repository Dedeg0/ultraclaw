...
const readBestEffortConfig = vi.fn(async () => ({
  gateway: {
    mode: "remote",
    remote: { url: "wss://remote.example:18790", token: "rtok" },
    auth: { token: "ltok" },
  },
}));
const resolveGatewayPort = vi.fn((_cfg?: unknown) => 18790);
...
describe("gateway-status command", () => {
  ...
  it("does not resolve local token SecretRef when ULTRACLAW_GATEWAY_TOKEN is set", async () => {
    const { runtime, runtimeLogs, runtimeErrors } = createRuntimeCapture();
    await withEnvAsync(
      {
        ...
        config: {
          ...
          gateway: {
            ...
            auth: {
              ...
              password: { source: "env", provider: "default", id: "ULTRACLAW_GATEWAY_PASSWORD" },
            },
            remote: {
    ...
    expect(configRemoteTarget?.config).toMatchInlineSnapshot(`
      {
        ...
        "gateway": {
          ...
          "remoteUrl": "wss://remote.example:18790",
          "tailscaleMode": null,
        },
        "issues": [],
        "legacyIssues": [],
  ...
  it("infers SSH target from gateway.remote.url and ssh config", async () => {
    ...
    await withEnvAsync({ USER: "steipete" }, async () => {
      readBestEffortConfig.mockResolvedValueOnce(
        makeRemoteGatewayConfig("ws://peters-mac-studio-1.sheep-coho.ts.net:18790"),
      );
  ...
  it("falls back to host-only when USER is missing and ssh config is unavailable", async () => {
    ...
    await withEnvAsync({ USER: "" }, async () => {
      readBestEffortConfig.mockResolvedValueOnce(
        makeRemoteGatewayConfig("wss://studio.example:18790"),
      );
