...
describe("backupCreateCommand atomic archive write", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("ultraclaw-backup-atomic-test-");
  ...
  it("does not leave a partial final archive behind when tar creation fails", async () => {
    ...
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-backup-failure-"));
    try {
  ...
  it("does not overwrite an archive created after readiness checks complete", async () => {
    const stateDir = path.join(tempHome.home, ".ultraclaw");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-backup-race-"));
  ...
  it("falls back to exclusive copy when hard-link publication is unsupported", async () => {
    const stateDir = path.join(tempHome.home, ".ultraclaw");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "ultraclaw-backup-copy-fallback-"));
    const linkSpy = vi.spyOn(fs, "link");
    try {
      await fs.writeFile(path.join(stateDir, "ultraclaw.json"), JSON.stringify({}), "utf8");
      await fs.writeFile(path.join(stateDir, "state.txt"), "state\n", "utf8");
