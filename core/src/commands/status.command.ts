...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByUltraClaw ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByUltraClaw ? "installed · " : "";
  ...
  runtime.log(theme.heading("UltraClaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`ultraclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("ultraclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("ultraclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("ultraclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("ultraclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.ultraclaw.os/faq");
  runtime.log("Troubleshooting: https://docs.ultraclaw.os/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("ultraclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("ultraclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("ultraclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("ultraclaw gateway probe")}`);
  }
}
