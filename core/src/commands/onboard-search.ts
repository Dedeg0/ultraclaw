...
function rawKeyValue(config: UltraClawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: UltraClawConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: UltraClawConfig, provider: SearchProvider): UltraClawConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: UltraClawConfig, result: UltraClawConfig): UltraClawConfig {
...
export async function setupSearch(
  ...
  const choice = await prompter.select<PickerValue>({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with ultraclaw configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — UltraClaw will store a reference instead of the API key.",
