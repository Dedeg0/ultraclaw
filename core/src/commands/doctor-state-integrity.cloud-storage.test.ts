...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "UltraClaw", ".ultraclaw");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/ultraclaw-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".ultraclaw");
  ...
  it("anchors cloud detection to OS homedir when ULTRACLAW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".ultraclaw");
    const originalUltraClawHome = process.env.ULTRACLAW_HOME;
    process.env.ULTRACLAW_HOME = "/tmp/ultraclaw-home-override";
    ...
    try {
      ...
      if (originalUltraClawHome === undefined) {
        delete process.env.ULTRACLAW_HOME;
      } else {
        process.env.ULTRACLAW_HOME = originalUltraClawHome;
      }
    }
  });
