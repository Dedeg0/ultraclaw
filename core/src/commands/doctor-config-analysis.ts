...
import { UltraClawSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: UltraClawConfig): {
  config: UltraClawConfig;
  removed: string[];
} {
  const parsed = UltraClawSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: UltraClawConfig): void {
