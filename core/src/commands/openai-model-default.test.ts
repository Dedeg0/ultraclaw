...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: UltraClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: UltraClawConfig) => { changed: boolean; next: UltraClawConfig };
  defaultModel: string;
  overrideConfig: UltraClawConfig;
  alreadyDefaultConfig: UltraClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: UltraClawConfig) => config,
      applyDefaultConfig: (config: UltraClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: UltraClawConfig = { agents: { defaults: {} } };
