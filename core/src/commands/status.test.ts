...
beforeAll(() => {
  envSnapshot = captureEnv(["ULTRACLAW_PROFILE"]);
  process.env.ULTRACLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/ultraclaw-root.js", () => ({
  resolveUltraClawPackageRoot: vi.fn().mockResolvedValue("/tmp/ultraclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/ultraclaw/pnpm-lock.yaml",
      markerPath: "/tmp/ultraclaw/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.ultraclaw.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.ultraclaw.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("ultraclaw status --all") ||
          line.includes("ultraclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("ULTRACLAW_GATEWAY_TOKEN", "abcd1234", async () => {
