...
export function requireUltraClawAgentDir(): string {
  const agentDir = process.env.ULTRACLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("ULTRACLAW_AGENT_DIR not set");
  }
  return agentDir;
}
