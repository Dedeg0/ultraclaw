...
const SEARCH_TOOL = "https://docs.ultraclaw.os/mcp.SearchUltraClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.ultraclaw.os");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('ultraclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.ultraclaw.os/");
      runtime.log(`Search: ${formatCliCommand('ultraclaw docs "your query"')}`);
    }
    return;
  }
