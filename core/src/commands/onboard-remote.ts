...
function buildLabel(beacon: GatewayBonjourBeacon): string {
  ...
  const port = beacon.port ?? beacon.gatewayPort ?? 18790;
...
function validateGatewayWebSocketUrl(value: string): string | undefined {
  ...
  if (
    !isSecureWebSocketUrl(trimmed, {
      allowPrivateWs: process.env.ULTRACLAW_ALLOW_INSECURE_PRIVATE_WS === "1",
    })
  ) {
    return (
      "Use wss:// for remote hosts, or ws://127.0.0.1/localhost via SSH tunnel. " +
      "Break-glass: ULTRACLAW_ALLOW_INSECURE_PRIVATE_WS=1 for trusted private networks."
    );
  }
  return undefined;
}
...
export async function promptRemoteGatewayConfig(
  ...
  if (!hasBonjourTool) {
    await prompter.note(
      [
        ...
        "Docs: https://docs.ultraclaw.os/gateway/discovery",
      ].join("\n"),
      "Discovery",
    );
  }
  ...
  if (selectedBeacon) {
    ...
    const port = selectedBeacon.port ?? selectedBeacon.gatewayPort ?? 18790;
    if (host) {
  ...
  if (authChoice === "token") {
    const selectedMode = await resolveSecretInputModeForEnvSelection({
      ...
      copy: {
        ...
        plaintextHint: "Stores the token directly in UltraClaw config",
      },
    });
    if (selectedMode === "ref") {
      const resolved = await promptSecretRefForOnboarding({
        ...
        config: cfg,
        prompter,
        preferredEnvVar: "ULTRACLAW_GATEWAY_TOKEN",
        copy: {
          sourceMessage: "Where is this gateway token stored?",
          envVarPlaceholder: "ULTRACLAW_GATEWAY_TOKEN",
        },
      });
      token = resolved.ref;
    } else {
    ...
    const selectedMode = await resolveSecretInputModeForEnvSelection({
      ...
      copy: {
        ...
        plaintextHint: "Stores the password directly in UltraClaw config",
      },
    });
    if (selectedMode === "ref") {
      const resolved = await promptSecretRefForOnboarding({
        ...
        config: cfg,
        prompter,
        preferredEnvVar: "ULTRACLAW_GATEWAY_PASSWORD",
        copy: {
          sourceMessage: "Where is this gateway password stored?",
          envVarPlaceholder: "ULTRACLAW_GATEWAY_PASSWORD",
        },
      });
