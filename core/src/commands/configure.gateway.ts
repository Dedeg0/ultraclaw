...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "ULTRACLAW_GATEWAY_TOKEN",
          placeholder: "ULTRACLAW_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like ULTRACLAW_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. UltraClaw will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: UltraClaw trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.ultraclaw.os/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
