...
describe("promptRemoteGatewayConfig", () => {
  const envSnapshot = captureEnv(["ULTRACLAW_ALLOW_INSECURE_PRIVATE_WS"]);
  ...
  it("allows ws:// hostname remote URLs when ULTRACLAW_ALLOW_INSECURE_PRIVATE_WS=1", async () => {
    process.env.ULTRACLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        expect(params.validate?.("ws://ultraclaw-gateway.ai:18790")).toBeUndefined();
        expect(params.validate?.("ws://1.1.1.1:18790")).toContain("Use wss://");
        return "ws://ultraclaw-gateway.ai:18790";
      }
      return "";
    }) as WizardPrompter["text"];
    ...
    expect(next.gateway?.mode).toBe("remote");
    expect(next.gateway?.remote?.url).toBe("ws://ultraclaw-gateway.ai:18790");
  });

  it("supports storing remote auth as an external env secret ref", async () => {
    process.env.ULTRACLAW_GATEWAY_TOKEN = "remote-token-value";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        return "wss://remote.example.com:18790";
      }
      if (params.message === "Environment variable name") {
        return "ULTRACLAW_GATEWAY_TOKEN";
      }
      return "";
    }) as WizardPrompter["text"];
