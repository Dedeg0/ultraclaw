---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for UltraClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, UltraClaw cron scheduling for periodic checks, or version status checks on a machine running UltraClaw (laptop, workstation, Pi, VPS).
---

# UltraClaw Host Hardening

## Overview

Assess and harden the host running UltraClaw, then align it to a user-defined risk tolerance without breaking access. Use UltraClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim UltraClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. UltraClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if UltraClaw can access sensitive data.
...
### 2) Run UltraClaw security audits (read-only)

As part of the default read-only checks, run `ultraclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `ultraclaw security audit` (faster, non-probing)
2. `ultraclaw security audit --json` (structured output)

Offer to apply UltraClaw safe defaults (numbered):

1. `ultraclaw security audit --fix`

Be explicit that `--fix` only tightens UltraClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check UltraClaw version/update status (read-only)

As part of the default read-only checks, run `ultraclaw update status`.
...
- Credential hygiene notes (location of UltraClaw creds, prefer disk encryption)
...
- UltraClaw security audit (re-run)
...
After UltraClaw install or first hardening pass, run at least one baseline audit and version check:

- `ultraclaw security audit`
- `ultraclaw security audit --deep`
- `ultraclaw update status`

Ongoing monitoring is recommended. Use the UltraClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `ultraclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `ultraclaw update status`
...
Before creating, `ultraclaw cron list` and match on exact `name`. If found, `ultraclaw cron edit <id> ...`.
If not found, `ultraclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `ultraclaw update status` (preferred for source checkouts and channels)
2. `npm view ultraclaw version` (published npm version)

## UltraClaw command accuracy

Use only supported commands and flags:

- `ultraclaw security audit [--deep] [--fix] [--json]`
- `ultraclaw status` / `ultraclaw status --deep`
- `ultraclaw health --json`
- `ultraclaw update status`
- `ultraclaw cron add|list|runs|run`

Do not invent CLI flags or imply UltraClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by UltraClaw compaction:
