...
metadata:
  {
    "ultraclaw": { "emoji": "🧩", "requires": { "anyBins": ["claude", "codex", "opencode", "pi"] } },
  }
---
...
**⚠️ CRITICAL: Never review PRs in UltraClaw's own project folder!**
...
8. **NEVER start Codex in ~/.ultraclaw/** - it'll read your soul docs and get weird ideas about the org chart!
9. **NEVER checkout branches in ~/Projects/ultraclaw/** - that's the LIVE UltraClaw instance!
...
For long-running background tasks, append a wake trigger to your prompt so UltraClaw gets notified immediately when the agent finishes (instead of waiting for the next heartbeat):

```
... your task here.

When completely finished, run this command to notify me:
ultraclaw system event --text "Done: [brief summary of what was built]" --mode now
```
...
When completely finished, run: ultraclaw system event --text \"Done: Built todos REST API with CRUD endpoints\" --mode now'"
```
